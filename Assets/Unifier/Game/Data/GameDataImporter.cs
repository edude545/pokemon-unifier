...
namespace Assets.Unifier.Game.Editor {
    ...
    [CreateAssetMenu(fileName = "Game Data Importer", menuName = "Unifier/Game Data Importer")]
    public class GameDataImporter : ScriptableObject {
        ...
        private void readCSV<T>(TextAsset csv, Func<Dictionary<string, string>, T> importFunction, Dictionary<string, T> dict) {
            string[] lines = getLines(csv);
            string[] fields = lines[0].Split("\t");
            for (int i = 1; i < lines.Length; i++) {
                Dictionary<string, string> record = buildRecordDict(fields, lines[i].Split("\t"));
                T asset = importFunction.Invoke(record);
        ...
        private static Species generateSpecies(Dictionary<string, string> record) {
        ...
        private static Move generateMove(Dictionary<string, string> record) {
            Move m = new Move();
            m.Name = record["name"];
