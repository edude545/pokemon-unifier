...
def exit(code=0)
  for p in $AtExitProcs
    ...
    $network.send("<DSC>") if $network rescue nil
...
module AudioState
  ...
  @bgsVolume=100.0
  @seVolume=100.0
  def self.setBgmVolume(volume)
    @bgmVolume=volume
  end
  def self.setMeVolume(volume)
    @meVolume=volume
  end
  def self.setBgsVolume(volume)
    @bgsVolume=volume
  end
  def self.setSeVolume(volume)
    @seVolume=volume
  end
...
def audio_control_volume(volume)
  #AudioState.volume=(volume)
  SoundEnvelope.changeDiscrete2(0,volume)
#  AudioState.setBgmVolume(volume)
#  AudioState.setMeVolume(volume)
#  AudioState.setSeVolume(volume)
#  AudioState.setBgsVolume(volume)
end
...
module Graphics
  ...
  def self.update
#      Kernel.registerMobile("-GMAudio1-")
    Audio.update
    audiomodule_update
 #     Kernel.registerMobile("-GMAudio2-")
  end
end
