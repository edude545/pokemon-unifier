        ...
        when "e"
          ...
          if enumer.is_a?(Array)
            ...
            if enumer.to_s=="PBTrainers" && !mod.respond_to?("getCount")
              file.write(pbGetTrainerConst(rec[i]))
            else
...
def pbCompileMetadata
  ...
  MessageTypes.setMessages(
     MessageTypes::StorageCreator,[sections[0][2]]
  )
end
...
def pbCompileItems
  ...
  pbCompilerEachCommentedLine("PBS/items.txt"){|line,lineno|
     ...
     record.push(linerecord[0])
     constant=linerecord[1]
     itemname=linerecord[2]
     constants+="#{constant}=#{record[0]}\r\n"
     itemnames[record[0]]=itemname
     record.push(itemname)
     record.push(linerecord[3])
     record.push(linerecord[4])
     itemdescs[record[0]]=linerecord[5]
     record.push(itemdescs[record[0]])
     record.push(linerecord[6])
     record.push(linerecord[7])
     record.push(linerecord[8])
     if linerecord[9]!="" && linerecord[9]
       record.push(parseMove(linerecord[9]))
     else
       record.push(0)
     end
     if linerecord[10]!="" && linerecord[10]
       record.push(linerecord[10])
     else
       record.push(100)
     end
     if linerecord[11]!="" && linerecord[11]
       record.push(linerecord[11])
     else
       record.push(101)
     end
...
def pbCompileMoves
  ...
  pbCompilerEachPreppedLine("PBS/moves.txt"){|line,lineno|
     ...
     begin
       ...
       begin
         oldrecord=pbGetCsvRecord(thisline,lineno,[0,"unss",
            nil,nil,nil,nil
         ])
         rescue
         raise $!.message+"\r\n"+oldmessage
...
def pbExtractTrainers
  ...
  File.open("trainertypes.txt","wb"){|f|
     ...
     for i in 0...trainertypes.length
       ...
       f.write(sprintf("%d,%s,%s,%d,%s,%s,%s,%s,%d\r\n",
...
def pbCompileTrainers
 ...
 #       raise _INTL("Bad nickname: {1} (must be 1-10 characters)\r\n{2}",poke[14],FileLineData.linereport) if (poke[14].to_s).length>10
      ...
      ### BEANS ###
      #poke[16]=strsplit(poke[16],/\s*;\s*/).collect{|s| s.to_i}
  ...
  pbCompileTrainersPokegear
  pbCompileTrainersEasy
  pbCompileTrainersHard
end


def pbCompileTrainersPokegear
  records=[]
  trainernames=[]
  count=0
  maxValue=0
  ...
  linenos=[]
  lineno=1
  File.open("PBS/trainers2.txt","rb"){|f|
     FileLineData.file="PBS/trainers2.txt"
     f.each_line {|line|
        line=prepline(line)
        if line!=""
          lines.push(line)
 ...
 #       raise _INTL("Bad nickname: {1} (must be 1-10 characters)\r\n{2}",poke[14],FileLineData.linereport) if (poke[14].to_s).length>10
  ...
  save_data(trainers,"Data/trainers2.dat")
end

def pbCompileTrainersEasy
  records=[]
  trainernames=[]
  count=0
  maxValue=0
  ...
  linenos=[]
  lineno=1
  File.open("PBS/trainers_easy.txt","rb"){|f|
     FileLineData.file="PBS/trainers_easy.txt"
     f.each_line {|line|
        line=prepline(line)
        if line!=""
          lines.push(line)
 ...
 #       raise _INTL("Bad nickname: {1} (must be 1-10 characters)\r\n{2}",poke[14],FileLineData.linereport) if (poke[14].to_s).length>10
  ...
  save_data(trainers,"Data/trainers_easy.dat")
end

def pbCompileTrainersHard
  records=[]
  trainernames=[]
  count=0
  maxValue=0
  ...
  linenos=[]
  lineno=1
  File.open("PBS/trainers_hard.txt","rb"){|f|
     FileLineData.file="PBS/trainers_hard.txt"
     f.each_line {|line|
        line=prepline(line)
        if line!=""
          lines.push(line)
 ...
 #       raise _INTL("Bad nickname: {1} (must be 1-10 characters)\r\n{2}",poke[14],FileLineData.linereport) if (poke[14].to_s).length>10
  ...
  save_data(trainers,"Data/trainers_hard.dat")
end
...
def pbCompileTypes
  ...
  for type in types
    ...
    for w in type[5]; if !typeinames.include?(w)
      raise _INTL("'{0}' is not a defined type (PBS/types.txt, {1}, Weaknesses)",w,n)
    end; end
    for w in type[6]; if !typeinames.include?(w)
      raise _INTL("'{0}' is not a defined type (PBS/types.txt, {1}, Resistances)",w,n)
    end; end
    for w in type[7]; if !typeinames.include?(w)
      raise _INTL("'{0}' is not a defined type (PBS/types.txt, {1}, Immunities)",w,n)
    end; end
  end
  for i in 0..maxValue
    if !typehash[i]
...
def pbCompilePokemonData
     ...
     pbEachFileSection(f){|lastsection,currentmap|
        ...
        [requiredtypes,optionaltypes].each{|hash|
           for key in hash.keys
             FileLineData.setSection(currentmap+1,key,lastsection[key])
             ...
             loop do
               ...
               for i in 0...schema.length
                 ...
                 if key=="EggMoves"
                   ...
                   raise _INTL("Kind {1} is greater than 13 characters long (section {2}, PBS/pokemon.txt)",value,currentmap) if value.length>13
 ...
 # code+="def PBSpecies.getName(id)\r\nif !$game_switches[354] || $Trainer.caught[id]\r\nreturn pbGetMessage(MessageTypes::Species,id)\r\nelse\r\nreturn \"??? ???\"\r\nend\r\nend\r\n"
  #code+="def PBSpecies.getName(id,bool=false)\r\nreturn pbGetMessage(MessageTypes::Species,id,bool)\r\nend\r\n" #glameow
  code+="def PBSpecies.getName(id)\r\nreturn pbGetMessage(MessageTypes::Species,id)\r\nend\r\n" #glameow
#  code+="def PBSpecies.getTrueName(id)\r\nreturn pbGetMessage(MessageTypes::Species,id)\r\nend\r\n"
  ...
  metrics[0].fillNils(dexdatas.length,8) # player Y
  metrics[1].fillNils(dexdatas.length,6) # enemy Y
  metrics[2].fillNils(dexdatas.length,0) # altitude
  save_data(metrics,"Data/metrics.dat")
  File.open("Data/regionals.dat","wb"){|f|
     offset=dexdatas.length*8
        ...
        if params[0][/SellItem\s*\(\s*(\w+)\s*\,\s*(\d+)\s*\)/]
          ...
          if hasConst?(PBItems,itemname)
            ...
            if cost==0
              ...
              pbPushScript(newEvents,_INTL("Kernel.pbReceiveItem(PBItems:"+":#{itemname})"),oldIndent+1)
              ...
              pbPushScript(newEvents,_INTL("Kernel.pbReceiveItem(PBItems:"+":#{itemname})"),oldIndent+2)
...
def pbConvertToTrainerEvent(event,trainerChecker)
  ...
  i=0;while i<list.length
    if list[i].code==108
      ...
      if command[/^(Battle\:|Type\:|Name\:|EndSpeech\:|VanishIfSwitch\:|EndBattle\:|RegSpeech\:|BattleID\:|EndIfSwitch\:|DoubleBattle\:|Backdrop\:|Sleepy\:|Angry\:|Nervous\:|Romantic\:)/i]
  ...
  if isFirstCommand && !event.name[/Trainer/] && !event.name[/Romantic/] && !event.name[/Sleepy/] && !event.name[/Angry/] && !event.name[/Nervous/]
    ret.name="Trainer(3)"
  elsif isFirstCommand && event.name[/^\s*Trainer\s+\((\d+)\)\s*$/] 
    ret.name="Trainer(#{$1})"
  elsif isFirstCommand && event.name[/^\s*Romantic\s+\((\d+)\)\s*$/] 
    ret.name="Romantic(#{$1})"
  elsif isFirstCommand && event.name[/^\s*Nervous\s+\((\d+)\)\s*$/] 
    ret.name="Nervous(#{$1})"
  elsif isFirstCommand && event.name[/^\s*Angry\s+\((\d+)\)\s*$/] 
    ret.name="Angry(#{$1})"
  elsif isFirstCommand && event.name[/^\s*Sleepy\s+\((\d+)\)\s*$/] 
    ret.name="Sleepy(#{$1})"
  end
  ...
  for command in commands
    ...
    if command[/^Sleepy\:\s*([\s\S]+)$/i]
      value=$~[1].gsub(/^\s+/,"").gsub(/\s+$/,"")
      sleepy=true if value.upcase=="TRUE" || value.upcase=="YES"
      pbPushComment(firstpage.list,command)
    end
    if command[/^Nervous\:\s*([\s\S]+)$/i]
      value=$~[1].gsub(/^\s+/,"").gsub(/\s+$/,"")
       nervous=true if value.upcase=="TRUE" || value.upcase=="YES"
      pbPushComment(firstpage.list,command)
    end
    if command[/^Romantic\:\s*([\s\S]+)$/i]
      value=$~[1].gsub(/^\s+/,"").gsub(/\s+$/,"")
      romantic=true if value.upcase=="TRUE" || value.upcase=="YES"
      pbPushComment(firstpage.list,command)
    end
    if command[/^Angry\:\s*([\s\S]+)$/i]
      value=$~[1].gsub(/^\s+/,"").gsub(/\s+$/,"")
      angry=true if value.upcase=="TRUE" || value.upcase=="YES"
      pbPushComment(firstpage.list,command)
    end
  end
  #angryTrainers=Array.new if angryTrainers==nil
  #sleepyTrainers=Array.new if sleepyTrainers==nil
  #nervousTrainers=Array.new if nervousTrainers==nil
  #romanticTrainers=Array.new if romanticTrainers==nil
  #angryTrainers.
  if battles.length<=0
    return nil
  end
  ...
  for i in 1...battles.length
    ...
    pbPushBranch(thirdpage.list,
       sprintf("pbTrainerBattle(%s,_I(\"%s\"),%s,%d)",
       safetrcombo,safequote2(espeech),doublebattle ? "true" : "false",battleid+i),1
    )
  ...
  for endswitch in vanishifswitch
    ...
    ret.pages.push(endIfSwitchPage)
      ...
      ret.name+=".angry" if angry
  ret.name+=".sleepy" if sleepy
  ret.name+=".nervous" if nervous
  ret.name+=".romantic" if romantic
