...
#          end
  ...
  def pbMoveDependentEvents
    ...
    leader=$game_player
    for i in 0...events.length
   #   return if events == nil || !events[i].is_a?(Array)
      event=@realEvents[i]
      pbFollowEventAcrossMaps(leader,event,false,i==0)
      # Update X and Y for this event
      events[i][3]=event.x if events[i] && events[i].is_a?(Array)
      events[i][4]=event.y if events[i] && events[i].is_a?(Array)
      events[i][5]=event.direction if events[i] && events[i].is_a?(Array)
      # Set leader to this event
      leader=event
    end
  end
