def getLineBrokenChunks(bitmap,value,width,dims,plain=false)
  ...
  while ((c = textmsg.slice!(/\n|(\S*([ \r\t\f]?))/)) != nil)
    ...
    for i in 0...words.length
      if prelength+bitmap.text_size(words[i]).width >= width-2
        prelength=bitmap.text_size(words[i]).width
        linecount+=1
      end
    end
    if linecount > 0
   #   bitmap.font.size /= 1.5
   #   thing=true
    end
    for i in 0...words.length
      word=words[i]
      if word && word!=""
        ...
        if x>0 && x+textwidth>=width-2
          x=0
          y+=32 if !thing # (textheight==0) ? bitmap.text_size("X").height : textheight
          y+=32/1.5 if thing
          textheight=0
        end
        textheight=32 # [textheight,textSize.height].max
        ret.push([word,x,y,textwidth,textheight,color])
        x+=textwidth if !thing
        x+=textwidth/1.5 if thing
...
def renderLineBrokenChunksWithShadow(bitmap,xDst,yDst,normtext,maxheight,
                                     baseColor,shadowColor,isSecond=false)
...
ret = []
#def getLineBrokenChunks(bitmap,value,width,dims,plain=false)
#[word,x,y,textwidth,textheight,color]
 # while 1==1
...
FORMATREGEXP=/<(\/?)([Cc]|[Cc][2]|[Cc][33]|[Oo]|[Ff][Nn]|[Bb][Rr]|[Ff][Ss]|[Ii]|[Bb]|[Uu]|[Ss]|[Ii][Cc][Oo][Nn]|[Ii][Mm][Gg]|[Aa][Cc]|[Aa][Rr]|[Aa][Ll]|[Rr]|[Oo][Uu][Tt][Ll][Nn]|[Oo][Uu][Tt][Ll][Nn][2])(\s*\=\s*([^>]*))?>/
  ...
  position=0;while position<textchars.length
    ...
    if heightDst<0 || texty<yDst+heightDst
      ...
      characters.push([
        ...
        (underlinecount>0),(strikecount>0),fontname,fontsize,position,graphicRect,
...
def drawTextEx(bitmap,x,y,width,numlines,text,baseColor,shadowColor,newnorm=false)
  normtext=getLineBrokenChunks(bitmap,text,width,nil,true)
 renderLineBrokenChunksWithShadow(bitmap,x,y,normtext,numlines*32,
     baseColor,shadowColor,newnorm)
  return []
end
