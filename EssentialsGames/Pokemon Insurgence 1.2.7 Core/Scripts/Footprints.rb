...
TERRAINFOOT=3
TERRAINFOOT2=PBTerrain::NoEncounterSandAndSnow
...
class Footprint
  def initialize(event,position)
    raceMaps=[274,345,346,568,272]
    return if !$scene || !$scene.is_a?(Scene_Map) ||
    event!=$game_player && (event.character_name=="" || event.character_name=="000" ||
    event.name.include?("/nofoot/")) || raceMaps.include?($game_map.map_id)
    return if event!=$game_player && pbEventCommentInput(event,0,"NoFoot")
    return if event.id==Kernel.pbGetDependency("Dependent").id
...
class Game_Character
  ...
  def increase_steps
    if terrain_tag_pos(get_last_pos[0],get_last_pos[1])==TERRAINFOOT || 
       terrain_tag_pos(get_last_pos[0],get_last_pos[1])==TERRAINFOOT2
...
class Footsprite
  def initialize(sprite,event,viewport,map,direction,nid,chardata,player)
    ...
    if $PokemonGlobal.bicycle && player
