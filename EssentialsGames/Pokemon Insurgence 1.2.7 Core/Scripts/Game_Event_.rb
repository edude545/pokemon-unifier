class Game_Event < Game_Character
  ...
  def start
    if $game_switches[368]==false && Kernel.getAllSBMaps.include?($game_map.map_id) && @id >= 1 && @id <= 100
      Kernel.runSBEvent($game_variables[78],@id)
    elsif @list.size > 1
      @starting = true
    end
  end
  ...
  def pbCheckEventTriggerAfterTurning
    ...
    if @event.name[/^Trainer\((\d+)\)$/] || @event.name[/^Romantic\((\d+)\)$/] || @event.name[/^Sleepy\((\d+)\)$/]
      #(@event.name.include?(".romantic") || @event.name.include?(".nervous") || @event.name.include?(".sleepy") || @event.name.include?(".angry"))
      distance=$~[1].to_i
      if @trigger == 2 and pbEventCanReachPlayer?(self,$game_player,distance)
         if not jumping? and not over_trigger? and not $PokemonGlobal.surfing
           start
         end
      end
    end
 ...
 #   Kernel.pbMessage(ontime.to_s) 
    return false if !ontime
    now=pbGetTimeNow
      #      Kernel.pbMessage((now.to_i-ontime).to_s+" no") 

    elapsed=(now.to_i-ontime)/86400
   #     Kernel.pbMessage(elapsed.to_s+" ho") 
  ...
  def refresh
    ...
    unless @erased
      for page in @event.pages.reverse
        ...
        if c.variable_valid
          if $game_variables[c.variable_id].instance_of?(String)
            c.variable_id = 0
          end
