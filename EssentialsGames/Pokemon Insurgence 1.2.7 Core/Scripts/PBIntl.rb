...
def pbSetTextMessages
  ...
  begin
    ...
    # Must add messages since this code is shared by game system and editor
...
def pbGetMessage(type,id,speciesnojumble=false)
  if type==MessageTypes::Species && (!$Trainer || ($Trainer && $Trainer.owned[id]) || speciesnojumble ||!$game_switches[354]) #glameow
  return MessageTypes.get(type,id)
elsif type==MessageTypes::Species
  return "??? ???"
else
    return MessageTypes.get(type,id)
...
def _INTL(*arg)
  ...
  string.gsub!("Pokemon","Pokémon")
  string.gsub!("Pokemart","Pokémart")
  string.gsub!("fiancee","fiancée")
  string.gsub!("fiance","fiancé")
  string.gsub!("Pokeflute","Pokéflute")
  string.gsub!("Pokerus","Pokérus")
  string.gsub!("Pokepon","Poképon")
  string.gsub!("Pokedex","Pokédex")
  string.gsub!("Pokegear","Pokégear")
  string.gsub!("Pokefan","Pokéfan")
  string.gsub!("cliched","clichéd")
  string.gsub!("forcefield","force field")


return scrambleMystery(string)
  end
...
def scrambleMystery(string)
   # return string if !PBSpecies
   begin
  for i in 0..PBSpecies.maxValue-1 
    string.gsub!(Kernel.pbGetMessage(MessageTypes::Species,i,true),"??? ???") if $game_switches[354] && $Trainer != nil && $Trainer.caught[i] == false
  end
    return string
  rescue
    return string
