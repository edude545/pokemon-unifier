class PokeBattle_BattleArena < PokeBattle_Battle
  ...
  def pbSwitch(favorDraws=false)
    ...
    switched=[]
    if @battlers[0].hp==0 && @partyindexes[0]+1<self.pbParty(0).length
      @partyindexes[0]+=1
    ...
    if @battlers[1].hp==0 && @partyindexes[1]+1<self.pbParty(1).length
      @partyindexes[1]+=1  
  ...
  def pbMindScore(move)
    if move.function==0xAA || # Detect/Protect
       move.function==0xAB || # Quick Guard
       move.function==0xAC || # Wide Guard
       move.function==0xE8 || # Endure
       move.function==0x14B || # King's Shield
       move.function==0x14C || # Spiky Shield
       move.function==0x176 || # Mat Block
  ...
  def pbEndOfRoundPhase
    ...
    if @count==3 && !@battlers[0].fainted? && !@battlers[1].fainted?
      points=[0,0]
