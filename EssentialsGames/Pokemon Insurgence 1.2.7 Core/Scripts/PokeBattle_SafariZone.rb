class PokeBattle_FakeBattler
  ...
  def fainted?; return @pokemon.hp<=0; end
      ...
      begin
        ...
        case cmd
          when 0 # Ball
            ...
            if safariBall
              pbThrowPokeBall(1,safariBall,rare)
            end
          when 1 # Rock
            pbDisplay(_INTL("{1} threw a rock at the wild {2}!",self.pbPlayer.name,wildpoke.name))
            g*=2 # Easier to catch
            ...
            lastCommand=2
          when 2 # Bait
            pbDisplay(_INTL("{1} threw bait at the wild {2}!",self.pbPlayer.name,wildpoke.name))
            g/=2 # Harder to catch
        ...
        if @decision==0
          if @ballcount<=0
            ...
            @decision=2
             ...
             pbDisplay(_INTL("Wild {1} fled!",wildpoke.name))
             @decision=3
          elsif lastCommand==1
             pbDisplay(_INTL("Wild {1} is eating!",wildpoke.name)) 
          elsif lastCommand==2
             pbDisplay(_INTL("Wild {1} is angry!",wildpoke.name)) 
          else
             pbDisplay(_INTL("Wild {1} is watching carefully!",wildpoke.name)) 
          end
        end
...
#     Graphics.update
  ...
  def pbDisplayBrief(msg)
    ...
    @scene.pbDisplayMessage(msg,$is_insane)
    end
  end
