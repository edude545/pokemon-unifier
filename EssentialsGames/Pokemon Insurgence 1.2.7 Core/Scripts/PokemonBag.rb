...
class Window_PokemonBag < Window_DrawableCommand
  ...
  def drawItem(index,count,rect)
    ...
    if index==@bag.pockets[self.pocket].length
      ...
      if !pbIsImportantItem?(item) || pbIsMegaStone?(item)#item==PBItems::AMPHAROSITE # Not a Key item or HM (or infinite TM)
#        Kernel.pbMessage("!")
    ...
    if index!=@bag.pockets[self.pocket].length
      @bag.setRegistered0 if @bag.registeredItem2==nil || @bag.registeredItem3==nil
      if @bag.registeredItem==@bag.pockets[self.pocket][index][0]
        pbDrawImagePositions(self.contents,[
           ["Graphics/Pictures/bagReg",rect.x+rect.width-58,ypos+4,0,0,-1,-1]
        ])
      end
            if @bag.registeredItem2==@bag.pockets[self.pocket][index][0]
        pbDrawImagePositions(self.contents,[
           ["Graphics/Pictures/bagReg2",rect.x+rect.width-58,ypos+4,0,0,-1,-1]
        ])
      end
      if @bag.registeredItem3==@bag.pockets[self.pocket][index][0]
        pbDrawImagePositions(self.contents,[
           ["Graphics/Pictures/bagReg3",rect.x+rect.width-58,ypos+4,0,0,-1,-1]
        ])
      end
      if @bag.registeredItem4==@bag.pockets[self.pocket][index][0]
        pbDrawImagePositions(self.contents,[
           ["Graphics/Pictures/bagReg4",rect.x+rect.width-58,ypos+4,0,0,-1,-1]
        ])
      end
      if @bag.registeredItem5==@bag.pockets[self.pocket][index][0]
        pbDrawImagePositions(self.contents,[
           ["Graphics/Pictures/bagReg5",rect.x+rect.width-58,ypos+4,0,0,-1,-1]
        ])
      end
      if $Trainer.clothes[0]==@bag.pockets[self.pocket][index][0]
        pbDrawImagePositions(self.contents,[
           ["Graphics/Pictures/bagHat",rect.x+rect.width-58,ypos+4,0,0,-1,-1]
        ])
      end
      
      if $Trainer.clothes[1]==@bag.pockets[self.pocket][index][0]
        pbDrawImagePositions(self.contents,[
           ["Graphics/Pictures/bagCoat",rect.x+rect.width-58,ypos+4,0,0,-1,-1]
        ])
      end
      
      if $Trainer.clothes[2]==@bag.pockets[self.pocket][index][0]
        pbDrawImagePositions(self.contents,[
           ["Graphics/Pictures/bagShirt",rect.x+rect.width-58,ypos+4,0,0,-1,-1]
        ])
      end
      if $Trainer.clothes[3]==@bag.pockets[self.pocket][index][0]
        pbDrawImagePositions(self.contents,[
           ["Graphics/Pictures/bagBackpack",rect.x+rect.width-58,ypos+4,0,0,-1,-1]
        ])
      end
      if $Trainer.clothes[4]==@bag.pockets[self.pocket][index][0]
        pbDrawImagePositions(self.contents,[
           ["Graphics/Pictures/bagPants",rect.x+rect.width-58,ypos+4,0,0,-1,-1]
        ])
      end
...
## Configuration
  ...
  def pbStartScene(bag)
    ...
    items=@bag.pockets[4]
    items.sort!{ |a,b| 
      ap = PBItems.getName(a[0]).split('M')
      ap[0] = ap[0].gsub(/H/,"Z")
      ap[0] = ap[0].gsub(/T/,"A")
      a = ap[0] + "%05d" % ap[1].to_i
      bp = PBItems.getName(b[0]).split('M')
      bp[0] = bp[0].gsub(/H/,"Z")
      bp[0] = bp[0].gsub(/T/,"A")
      b = bp[0] + "%05d" % bp[1].to_i
      a <=> b
    }
      
      
    #  .scan(/\d+/)[0].to_i <=> PBItems.getName(b[0]).scan(/\d+/)[0].to_i }
  ...
  def pbRefresh
    ...
    filename=sprintf("Graphics/Icons/itemBack") if itemwindow.item==0
  ...
  def pbChooseItem
    ...
    pbActivateWindow(@sprites,"itemwindow"){
       loop do
         ...
         # Sort pocket if F5 is pressed
         numpockets=PokemonBag.numPockets
         if Input.trigger?(Input::F5)
           if @bag.pockets[itemwindow.pocket].length>0
             @bag.pbMassSort(itemwindow.pocket)
             if itemwindow.index>@bag.pockets[itemwindow.pocket].length
               itemwindow.index=0
             end
             pbRefresh
           end
...
class PokemonBag
  ...
  attr_reader :registeredItem2
  attr_reader :registeredItem3
  attr_reader :registeredItem4
  attr_reader :registeredItem5
  ...
  def initialize
    @lastpocket=1
    @pockets=[] # if !$game_switches[4]
    @choices=[]# if !$game_switches[4]
    # Initialize each pocket of the array
    for i in 0..PokemonBag.numPockets
      @pockets[i]=[] #if !$game_switches[4]
      @choices[i]=0 #if !$game_switches[4]
    end
    @registeredItem=0
    @registeredItem2=0
    @registeredItem3=0
...
# Registers the item as a key item.  Can be retrieved with $PokemonBag.registeredItem
  def pbRegisterKeyItem(item)
    ...
    def pbRegisterKeyItem2(item)
    if item!=@registeredItem2
      @registeredItem2=item
    else
      @registeredItem2=0
    end
  end
  def pbRegisterKeyItem3(item)
    if item!=@registeredItem3
      @registeredItem3=item
    else
      @registeredItem3=0
    end
  end

  def pbRegisterKeyItem4(item)
    if item!=@registeredItem4
      @registeredItem4=item
    else
      @registeredItem4=0
    end
  end
  
  def pbRegisterKeyItem5(item)
    if item!=@registeredItem5
      @registeredItem5=item
    else
      @registeredItem5=0
    end
  end
  ...
  def pbDeleteItem(item,qty=1)
    ...
    if ret
      ...
      @registeredItem2=0 if @registeredItem2==item && pbQuantity(item)<=0
      @registeredItem3=0 if @registeredItem3==item && pbQuantity(item)<=0
    end
    return ret
  end
  ...
  def pbCanStoreMultiple?(items,qty=1)
    ret=true
    for i in 0...items.length
      pocket=pbGetPocket(items[i])
      maxsize=maxPocketSize(pocket)
      maxsize=@pockets[pocket].length+1 if maxsize<0
      if !ItemStorageHelper.pbCanStore?(
         @pockets[pocket],maxsize,BAGMAXPERSLOT,items[i],qty)
        ret=false
        break
      end
    end
  ...
  def pbMassSort(pocket)
    return ItemStorageHelper.pbMassSort(
       @pockets[pocket])
  end
end
...
# UI logic for the item screen when an item is to be held by a Pokémon.
  def pbGiveItemScreen
    ...
    loop do
      ...
      if pbIsImportantItem?(item) && !pbIsMegaStone?(item) && !$DEBUG
        @scene.pbDisplay(_INTL("The {1} can't be held.",itemname))
        next
      elsif (@bag.lastpocket!=6 && pbIsMegaStone?(item)) ||
            (@bag.lastpocket!=8 && pbIsKeyItem?(item))
        @scene.pbDisplay(_INTL("The {1} needs to be sorted first.",itemname))
        next
      else
        break
      end
    ...
    loop do
      ...
      if command==0
        ...
        if $Trainer.pokemonCount==0
          ...
          return 0
        elsif pbIsImportantItem?(item) && !pbIsMegaStone?(item)#item!=PBItems::AMPHAROSITE
          @scene.pbDisplay(_INTL("The {1} can't be held.",itemname))
        else
          pbFadeOutIn(99999){
...
# UI logic for depositing an item in the item screen.
  ...
  def pbStartScreen
    ...
    loop do
      ...
      commands[cmdGive=commands.length]=_INTL("Give") if $Trainer.party.length>0 && (!pbIsImportantItem?(item) || pbIsMegaStone?(item))#item==PBItems::AMPHAROSITE)
      commands[cmdToss=commands.length]=_INTL("Toss") if !pbIsImportantItem?(item) || $DEBUG
      if @bag.registeredItem==item || @bag.registeredItem2==item || @bag.registeredItem3==item || @bag.registeredItem4==item || @bag.registeredItem5==item
      ...
      if cmdUse>=0 && command==cmdUse # Use item
        ...
        if @bag.registeredItem==item || @bag.registeredItem2==item || @bag.registeredItem3==item || @bag.registeredItem4==item || @bag.registeredItem5==item
                  @bag.pbRegisterKeyItem(item) if @bag.registeredItem==item
        @bag.pbRegisterKeyItem2(item) if @bag.registeredItem2==item
        @bag.pbRegisterKeyItem3(item) if @bag.registeredItem3==item
        @bag.pbRegisterKeyItem4(item) if @bag.registeredItem4==item
        @bag.pbRegisterKeyItem5(item) if @bag.registeredItem5==item

          else
        tempcommand=@scene.pbShowCommands(_INTL("Register to which key?"),["Q Key","W Key","E Key","T Key","Y Key"])
        @bag.pbRegisterKeyItem(item) if tempcommand == 0
        @bag.pbRegisterKeyItem2(item) if tempcommand == 1
        @bag.pbRegisterKeyItem3(item) if tempcommand == 2
        @bag.pbRegisterKeyItem4(item) if tempcommand == 3
        @bag.pbRegisterKeyItem5(item) if tempcommand == 4
        end
        @scene.pbRefresh
