...
class BattleSwapScreen
  ...
  def pbStartSwap(currentPokemon,newPokemon,opponentItems)
          ...
          loop do
            ...
            if pkmn>=0
              if @scene.pbConfirm(_INTL("Accept this Pokémon?"))
                @scene.pbEndScene
                oldMon=currentPokemon[yourPkmn]
                newMon=newPokemon[pkmn]
                currentPokemon[yourPkmn]=newPokemon[pkmn]
                currentPokemon[yourPkmn].item=opponentItems[pkmn]
                return true
              end
            elsif pkmn==-2
