...
def pbBerryPlant
  ...
  watering=hasConst?(PBItems,:SPRAYDUCK) ? getConst(PBItems,:SPRAYDUCK) : 0
  watering=hasConst?(PBItems,:SQUIRTBOTTLE) ? getConst(PBItems,:SQUIRTBOTTLE) : watering
  watering=hasConst?(PBItems,:WAILMERPAIL) ? getConst(PBItems,:WAILMERPAIL) : watering
  berry=berryData[1]
  case berryData[0]
    when 0  # empty
  ...
  case berryData[0]
    when 1, 2, 3, 4
      if watering>0 && ($PokemonBag.pbQuantity(watering)>0)
        if Kernel.pbConfirmMessage(_INTL("Would you like to water the {1} plant with the {2}?",
           PBItems.getName(berryData[1]),PBItems.getName(watering)))
          if berryData[2]==false
            berryData[4]+=1
            berryData[2]=true
          end
