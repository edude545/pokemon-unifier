...
module Input
  ...
  def self.getstate(key)
    if @GetKeyState && @GetKeyState.call(key) && @GetKeyState.call(key)&0x8000
    return (@GetKeyState.call(key)&0x8000)>0 rescue false
  else
    return false
  end
  ...
  def self.update
    ...
    if @keystate
      ...
      for i in 0..256 #for i in 0...256
        @stateUpdated[i]=true rescue nil
        if i>12
          @keystate[i]=0
        else
          @keystate[i]=(self.getstate(i) ? 1 : 0 rescue 0)
        end
        @triggerstate[i]=false rescue nil
        @releasestate[i]=false rescue nil
      end
    end
  ...
  def self.buttonToKey(button)
    ...
    case button
      when Input::DOWN
        return !MBLVAR ? [0x28] : [0x28]  # Down
      when Input::LEFT
        return !MBLVAR ? [0x25] : [0x25]  # Left
      when Input::RIGHT
        return !MBLVAR ? [0x27] : [0x27]  # Right
      when Input::UP
        return !MBLVAR ? [0x26] : [0x26]  # Up
      when Input::A
        return !MBLVAR ? [0x5A,0x10] : [0x5A]  # Z, Shift
      when Input::B
        return !MBLVAR ? [0x58,0x1B] : [0x58]  # X, ESC 
      when Input::C
        return !MBLVAR ? [0x43,0x0d,0x20] : [0x43]  # C, ENTER, Space
      when Input::X
        return !MBLVAR ? [0x41] : []  # A
      when Input::Y
        return !MBLVAR ? [0x53] : []  # S
      when Input::Z
        return !MBLVAR ? [0x44] : []  # D
      when Input::L
        return !MBLVAR ? [0x51,0x21] : []  # Q, Page Up
      when Input::R
        return !MBLVAR ? [0x57,0x22] : []  # W, Page Down
      when Input::SHIFT
        return !MBLVAR ? [0x10] : [] # Shift
      when Input::CTRL
        return !MBLVAR ? [0x11] : [] # Ctrl
      when Input::ALT
        return !MBLVAR ? [0x12] : [] # Alt
      when Input::F5
        return !MBLVAR ? [0x74] : [] # F5
      when Input::F6
        return !MBLVAR ? [0x75] : [] # F6
      when Input::F7
        return !MBLVAR ? [0x76] : [] # F7
      when Input::F8
        return !MBLVAR ? [0x77] : [] # F8
      when Input::F9
        return !MBLVAR ? [0x78] : [] # F9
      else
        return []
    end
  end
