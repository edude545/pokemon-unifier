  ...
  File.open("PBS/shadowmoves.txt","wb"){|f|
     for i in 0...moves.length
       ...
       if move && moves.length>0
         constname=(pbGetSpeciesConst(i) rescue nil)
         next if !constname
         f.write(sprintf("%s=",constname))
         movenames=[]
         for m in move
           movenames.push((pbGetMoveConst(m) rescue nil))
         end
...
def pbSaveMoveData()
  ...
  File.open("PBS/moves.txt","wb"){|f|
     for i in 1..(PBMoves.maxValue rescue PBMoves.getCount-1 rescue pbGetMessageCount(MessageTypes::Moves)-1)
       moveconst=pbGetMoveConst(i) rescue nil
       ...
       f.write(sprintf("%d,%s,%s,%03X,%d,%s,%s,%d,%d,%d,%02X,%d,%s,%s,%s\r\n",
          i,csvquote(moveconst),csvquote(movename),
          movedata.function,
          movedata.basedamage,
          csvquote((pbGetTypeConst(movedata.type) rescue "")),
          ...
          movedata.priority,
          flags,
          csvquote(["Cool","Beauty","Cute","Smart","Tough"][movedata.contestType]),
...
def pbSaveItems
  ...
  File.open("PBS/items.txt","wb"){|f|
     for i in 0...itemData.length
       next if !itemData[i]
       data=itemData[i]
       cname=pbGetItemConst(i) rescue ""
       next if !cname || cname=="" || data[0]==0
       f.write(sprintf("%d,%s,%s,%d,%d,%s,%d,%d,%d,%s\r\n",
          data[0],csvquote(cname),csvquote(data[1]),
          data[2],data[3],csvquote(data[4]),
          data[5],data[6],data[7],
          data[8]==0 ? "" : csvquote((pbGetMoveConst(data[8]) rescue ""))
       ))
     end
 }
end
...
def pbFastInspect(pkmn,moves,species,items,natures)
  c1=species[pkmn.species] ? species[pkmn.species] :
     (species[pkmn.species]=pbGetSpeciesConst(pkmn.species))
  c2=items[pkmn.item] ? items[pkmn.item] :
     (items[pkmn.item]=pbGetItemConst(pkmn.item))
  ...
  c4=moves[pkmn.move1] ? moves[pkmn.move1] :
     (moves[pkmn.move1]=pbGetMoveConst(pkmn.move1))
  c5=moves[pkmn.move2] ? moves[pkmn.move2] :
     (moves[pkmn.move2]=pbGetMoveConst(pkmn.move2))
  c6=moves[pkmn.move3] ? moves[pkmn.move3] :
     (moves[pkmn.move3]=pbGetMoveConst(pkmn.move3))
  c7=moves[pkmn.move4] ? moves[pkmn.move4] :
     (moves[pkmn.move4]=pbGetMoveConst(pkmn.move4))
  ...
  File.open(filename,"wb"){|f|
     for i in 0...bttrainers.length
       ...
       for key in btTrainersRequiredTypes.keys
         ...
         f.write(sprintf("%s=",key))
         if key=="Type"
           f.write(pbGetTrainerConst(record))
...
def pbSaveMachines
  ...
  File.open("PBS/tm.txt","wb"){|f|
     for i in 1...machines.length
       ...
       next if !machines[i]
       movename=pbGetMoveConst(i) rescue nil
       next if !movename || movename==""
       f.write(sprintf("[%s]\r\n",movename))
       x=[]
       for j in 0...machines[i].length
         speciesname=pbGetSpeciesConst(machines[i][j]) rescue nil
       ...
       if encdata[i]
         ...
         for j in 0...e[1].length
           ...
           for k in 0...EncounterTypes::EnctypeChances[j].length
             ...
             species=pbGetSpeciesConst(encentry[0])
...
def pbSaveTrainerTypes()
  ...
  File.open("PBS/trainertypes.txt","wb"){|f|
     ...
     for i in 0...data.length
       ...
       if record
         begin
           constName=pbGetTrainerConst(i)
          rescue RuntimeError
           constName="TRAINER#{i}"
         end
         dataline=sprintf("%d,%s,%s,%d,%s,%s,%s,%s,%d\r\n",
            i,constName,record[2],
            record[3],
            record[4] ? record[4] : "",
            record[5] ? record[5] : "",
            record[6] ? record[6] : "",
            record[7] ? ["Male","Female","Mixed"][record[7]] : "Mixed",
            (record[8]!=record[3]) ? record[8] : 0
         )
         f.write(dataline)
       end
     end
...
def pbSaveTrainerBattles()
  ...
  if data.is_a?(Array)
    File.open("PBS/trainers.txt","wb"){|f|
       ...
       f.write("\r\n")
       for trainer in data
         trname=pbGetTrainerConst(trainer[0]) rescue nil
         next if !trname
         f.write("\#-------------------\r\n")
         f.write(sprintf("%s\r\n",trname))
         ...
         for i in 0...3
           f.write(sprintf(",%s",pbGetItemConst(trainer[2][i]))) if trainer[2][i]
         end
         f.write("\r\n")
         for poke in trainer[3]
           hasmoves=(poke[3]>0 || poke[4]>0 || poke[5]>0 || poke[6]>0)
           f.write(sprintf("%s,%d",pbGetSpeciesConst(poke[0]),poke[1]))
           if poke[2]>0 || hasmoves || poke[7] || poke[8] || poke[9]>0 || poke[10] ||
              poke[11] || poke[12]!=10 || poke[13]!=70 || poke[14] || poke[15]
             if poke[2]>0
               f.write(sprintf(",%s",pbGetItemConst(poke[2])))
             else
               f.write(",")
             end
             if hasmoves
               ...
               for i in 3...7
                 if poke[i] && poke[i]!=0
                   f.write(sprintf(",%s",pbGetMoveConst(poke[i])))
                 else
                   extra+=","
                 end
               end
             ...
             f.write(","+(poke[7] ? poke[7].to_s : ""))
             f.write(","+(poke[8] ? poke[8].to_s : ""))
             f.write(","+(poke[9] ? poke[9].to_s : ""))
             f.write(","+(poke[10] ? "true" : "false"))
             f.write(","+(poke[11] ? getConstantName(PBNatures,poke[11]) : ""))
             f.write(","+(poke[12] ? poke[12].to_s : ""))
             f.write(","+(poke[13] ? poke[13].to_s : ""))
             f.write(sprintf(",%s",poke[14]))
             f.write(","+(poke[15] ? "true" : "false"))
           end
           f.write("\r\n")
         end
       end
    ...
    pbRgssOpen("Data/trainers.dat","rb"){|f|
       File.open("PBS/trainers.txt","wb"){|fw|
          fw.write("\# "+_INTL("See the documentation on the wiki to learn how to edit this file."))
          fw.write("\r\n")
          numtrainers=f.fgetdw
          f.fgetdw
          curpos=f.pos
          for i in 0...numtrainers
            f.pos=curpos
            offset=f.fgetdw
            curpos=f.pos
            f.pos=offset
            ...
            namelength=f.fgetw
            f.pos=nameoffset
            name=f.read(namelength)
            f.pos=offset+6
            trainer=f.fgetw
            thispartyid=f.fgetb
            trname=pbGetTrainerConst(trainer) rescue nil
            next if !trname
            numpoke=f.fgetb
            numitems=f.fgetb
            items=[]
            ...
            fw.write("\#-------------------\r\n")
            fw.write(sprintf("%s\r\n",trname))
            trainername=name ? name.gsub(/,/,";") : "???"
            if thispartyid==0
              fw.write(sprintf("%s\r\n",trainername))
            else
              fw.write(sprintf("%s,%d\r\n",trainername,thispartyid))
            end
            fw.write(sprintf("%d",numpoke))
            for i in 0...3
              fw.write(sprintf(",%s",pbGetItemConst(items[i]))) if items[i]
            end
            fw.write("\r\n")
            for j in 0...numpoke
              species=f.fgetw
              ...
              iv=f.fgetb
              item=f.fgetw
              poke=[species,level,iv&0x1F,item,0,0,0,0]
              if (iv&0x80)!=0
                for k in 0...4
                  poke[4+k]=f.fgetw
                end
              end
              hasmoves=(poke[4]>0||poke[5]>0||poke[6]>0||poke[7]>0)
              fw.write(sprintf("%s,%d",pbGetSpeciesConst(poke[0]),poke[1]))
              if poke[2]!=10 || poke[3]>0 || hasmoves || poke[8] || poke[9]
                if poke[3]>0
                  fw.write(sprintf(",%d,%s",poke[2],pbGetItemConst(poke[3]))) 
                else
                  fw.write(sprintf(",%d,",poke[2])) 
                end
                if hasmoves
                  extra=""
                  for i in 4...8
                    if poke[i] && poke[i]!=0
                      fw.write(sprintf(",%s",pbGetMoveConst(poke[i])))
                    else
                      extra+=","
                    end
                  end
...
def pbSavePokemonData
  ...
  for i in 1..(PBSpecies.maxValue rescue PBSpecies.getCount-1 rescue messages.getCount(MessageTypes::Species)-1)
    ...
    item3=dexdata.fgetw
    pokedata.write("[#{i}]\r\nName=#{speciesname}\r\n")
    pokedata.write("InternalName="+pbGetSpeciesConst(i)+"\r\n")
    pokedata.write("Type1="+(pbGetTypeConst(type1) || pbGetTypeConst(0) || "NORMAL")+"\r\n")
    if type1!=type2
      pokedata.write("Type2="+(pbGetTypeConst(type2) || pbGetTypeConst(0) || "NORMAL")+"\r\n")
    end
    ...
    elsif ability1==0 && ability2!=0
      pokedata.write("Abilities="+pbGetAbilityConst(ability2)+"\r\n")
    else
      pokedata.write("Abilities="+pbGetAbilityConst(ability1))
      pokedata.write(","+pbGetAbilityConst(ability2)) if ability2!=0
      pokedata.write("\r\n")
    end
    if hiddenability>0
      pokedata.write("HiddenAbility="+pbGetAbilityConst(hiddenability)+"\r\n")
    end
    ...
    for j in 0...movelist.length
      ...
      pokedata.write(sprintf("%d,%s",alevel,pbGetMoveConst(move)))
    end
    pokedata.write("\r\n")
    ...
    if length>0
      ...
      j=0; loop do break unless j<length
        ...
        break if atk==0
        if atk>0
          pokedata.write(pbGetMoveConst(atk))
          first=false
        end
        j+=1
      end
    ...
    pokedata.write("\r\n")
    pokedata.write("Compatibility=#{compat1},#{compat2}\r\n")
    pokedata.write("StepsToHatch=#{stepstohatch}\r\n")
    pokedata.write(sprintf("Height=%.1f\r\n",height/10.0)) if height
    pokedata.write(sprintf("Weight=%.1f\r\n",weight/10.0)) if weight
    ...
    pokedata.write("WildItemCommon="+pbGetItemConst(item1)+"\r\n") if item1>0
    pokedata.write("WildItemUncommon="+pbGetItemConst(item2)+"\r\n") if item2>0
    pokedata.write("WildItemRare="+pbGetItemConst(item3)+"\r\n") if item3>0
    if metrics
    ...
    evotypes=["Unknown",
       ...
       "Custom1","Custom2","Custom3","Custom4","Custom5","Custom6","HappinessFairy",
       "Unhappiness","LevelDay","LevelNight","LevelRain","LevelDarkInParty"
    ]
    count=0
    for form in pbGetEvolvedFormData(i)
      ...
      pokedata.write(sprintf("%s,%s,",pbGetSpeciesConst(poke),evotypes[evonib],evonib))
      evoparams=[0,
         0,0,0,1,0,
         2,2,1,1,1,
         1,1,1,1,1,
         ...
         5,5,1,1,1,
         1]
      case evoparams[evonib]
        when 1
          pokedata.write(_ISPRINTF("{1:d}",level))
        when 2
          pokedata.write(sprintf("%s",pbGetItemConst(level)))
        when 3
          pokedata.write(sprintf("%s",pbGetMoveConst(level)))
        when 4
          pokedata.write(sprintf("%s",pbGetSpeciesConst(level)))
        when 5
          pokedata.write(_ISPRINTF("{1:d}",level))
      end
      count+=1
    ...
    pokedata.write("\r\n")
    if i%20==0
      Win32API.SetWindowText(_INTL("Processing {1}",i))
      Graphics.update
    end
  end
  eggEmerald.close
...
def pbChooseItemList(defaultItemID=0)
  ...
  if defaultItemID>0
    commands.each_with_index {|item,index|
       moveDefault=index if index[0]==defaultItemID
    }
  end
  realcommands=[]
  for command in commands
...
# selected or 0 if the selection was canceled.  defaultMoveID, if specified,
  ...
  loop do
    ...
    for i in 0...enctype.length
      if enctype[i][1]==enctype[i][2]
        commands.push(_INTL("{1}% {2} (L{3})",
           chances[i],
           PBSpecies.getName(enctype[i][0]),
           enctype[i][1]
        ))
      else
        commands.push(_INTL("{1}% {2} (L{3}-L{4})",
           chances[i],
           PBSpecies.getName(enctype[i][0]),
           enctype[i][1],
           enctype[i][2]
...
def pbCommands2(cmdwindow,commands,cmdIfCancel,defaultindex=nil,noresize=false)
  cmdwindow.z=99999
  ...
  cmdwindow.index=defaultindex if defaultindex
  ret=0
  command=0
  loop do
    Graphics.update
...
def pbEncounterEditorMap(encdata,map)
  ...
  loop do
    ...
    if !enc
      enc=[[25,10,10,0,0,0,0,0,0,25,25,25,25],[]]
      encdata[map]=enc
    end
    if choice==-2
...
class ItemLister
  ...
  def commands   # Sorted alphabetically
    ...
    for i in cmds
      @commands.push(_ISPRINTF("{1:03d}: {2:s}",i[0],i[1]))  # Number: Item name
      @ids.push(i[0])
    end
    @index=@selection
...
=end
  ...
  def refresh(index)
    ...
    return if index<0
    begin
      @sprite.setBitmap(sprintf("Graphics/Icons/item%03d",@ids[index]),0)
      rescue
...
class EnumProperty
  ...
  def set(settingname,oldsetting)
    ...
    cmd=Kernel.pbMessage(_INTL("Choose a value for {1}.",settingname),
       commands,-1)
    if cmd<0
      return oldsetting
    else
...
module StringProperty
  def self.set(settingname,oldsetting)
    message=Kernel.pbMessageFreeText(_INTL("Set the value for {1}.",settingname),
       oldsetting ? oldsetting : "",false,256)
  end
...
module WeatherEffectProperty
  def self.set(settingname,oldsetting)
    ...
    if cmd==0
      ...
      number=Kernel.pbMessageChooseNumber(_INTL("Set the effect chance."),params)
      return [cmd,number]
    end
  end
...
module MapCoordsFacingProperty
  def self.set(settingname,oldsetting)
    ...
    if chosenmap>=0
      mappoint=chooseMapPoint(chosenmap)
      if mappoint
        facing=Kernel.pbMessage(_INTL("Choose a facing."),
...
module MapSizeProperty
  def self.set(settingname,oldsetting)
    ...
    properties=[
       [_INTL("Width"),LimitProperty.new(30),
           _INTL("The width of this map in Region Map squares.")],
       [_INTL("Valid Squares"),StringProperty,
           _INTL("String marking which squares are part of this map (1=part, 0=not part).")],
    ]
...
class MapScreenScene
  ...
  def removeOldConnections(ret,mapid)
    for i in 0...ret.length
      if ret[i][0]==mapid || ret[i][3]==mapid
        ret[i]=nil
      end
    end
    ret.compact!
...
# Returns the maps within _keys_ that are directly connected to this map, _map_.
  ...
  def putSprite(id)
    mapsprite=getMapSprite(id)
    x=240-(mapsprite.bitmap.width/2)
    y=160-(mapsprite.bitmap.height/2)
  ...
  def getMetadata(mapid,metadataType)
    if @metadata[mapid]
      return @metadata[mapid][metadataType]
    end
  end
  ...
  def helpWindow
    ...
    helptext+=_INTL("Drag to move maps\r\n")
    ...
    helptext+=_INTL("Canvas can be dragged to view connected maps better")
...
GLOBALMETADATA=[
   ...
   ["StorageCreator",StringProperty,
       _INTL("Creator of the Pokémon Storage System.")],
   ["WildBattleBGM",BGMProperty,
   ...
   ["TextSkin",WindowskinProperty,
       _INTL("Default windowskin for speech messages.")],
   ["SurfBGM",BGMProperty,
  ...
  ["PlayerJ",PlayerProperty,
       ...
       ["PlayerM",PlayerProperty,
       _INTL("Specifies player M.")],
       ["PlayerN",PlayerProperty,
       _INTL("Specifies player N.")],
       ["PlayerO",PlayerProperty,
       _INTL("Specifies player O.")],
       ["PlayerP",PlayerProperty,
       _INTL("Specifies player P.")],
       ["PlayerQ",PlayerProperty,
       _INTL("Specifies player N.")],
              ["PlayerR",PlayerProperty,
       _INTL("Specifies player R.")],

       ["PlayerS",PlayerProperty,
       _INTL("Specifies player S.")]
]
...
#   echo("mousemove (#{mapid},#{x},#{y})\r\n")
  ...
  def update
    ...
    if Input.triggerex?("A"[0])
      ...
      if @selmapid>=0
        pbEncounterEditorMap(@encdata,@selmapid)
      end
...
def pbTrainerTypeEditorNew(trconst)
  ...
  if trname=="" && !trconst
    ...
    if !trconst
      ...
      while hasConst?(PBTrainers,trconst)
        trconst=sprintf("%s_%03d",trconst,trainertype)
      end
    else
      # Ensure valid trconst
      trconst=trname.gsub(/[^A-Za-z0-9_]/,"")
      trconst=trconst.sub(/^([a-z])/){ $1.upcase } 
      if trconst.length==0
        trconst=sprintf("T_%03d",trainertype)
    ...
    record[8] = record[3]
    PBTrainers.const_set(trconst,record[0])
    data[record[0]]=record
    save_data(data,"Data/trainertypes.dat")
    pbConvertTrainerData
    Kernel.pbMessage(_INTL("The Trainer type was created."))
    Kernel.pbMessage(
       _ISPRINTF("Put the Trainer's graphic (trainer{1:03d}.png) in Graphics/Characters, or it will be blank.",
       record[0]))
    return record[0]
  end
end
...
module TrainerPokemonProperty
  def self.set(settingname,oldsetting)
    oldsetting=[0,10,0,0,0,0,0,nil,nil,0,false,nil,10,nil,nil,false] if !oldsetting
    properties=[
       ...
       [_INTL("Ability"),LimitProperty.new(2),
           _INTL("Ability flag. 0=first ability, 1=second ability, 2=hidden ability.")],
       [_INTL("Gender"),LimitProperty.new(1),
           _INTL("Gender flag. 0=male, 1=female.")],
       [_INTL("Form"),LimitProperty.new(27),
...
#    for i in 0...15
#      next if i==10
#      oldsetting[i]=0 if !oldsetting[i]
#    end
    moves=[]
    moves.push(oldsetting[3]) if oldsetting[3]!=0
    moves.push(oldsetting[4]) if oldsetting[4]!=0
    moves.push(oldsetting[5]) if oldsetting[5]!=0
    moves.push(oldsetting[6]) if oldsetting[6]!=0
    oldsetting[3]=moves[0] ? moves[0] : 0 
    oldsetting[4]=moves[1] ? moves[1] : 0
    oldsetting[5]=moves[2] ? moves[2] : 0
    oldsetting[6]=moves[3] ? moves[3] : 0
    oldsetting=nil if !oldsetting[0] || oldsetting[0]==0
    return oldsetting
  end

  def self.format(value)
    return (!value || !value[0] || value[0]==0) ? "-" : PBSpecies.getName(value[0])
  end
end
         ...
         if selection<0
           ...
           while true
             ...
             if data
               trainerdata=[
                  data[0],data[1],
                  [data[9],data[10],data[11],data[12]].find_all {|i| i && i!=0 }, # Move list
                  [data[3],data[4],data[5],
                  data[6],data[7],data[8]].find_all {|i| i && i!=0 }, # Pokemon list
                  data[2]
               ]
...
def pbPokemonEditor
  ...
  trainerTypes=[
     ...
     ["Kind",LimitStringProperty.new(13),
         _INTL("Kind of Pokémon species.")],
     ["Pokédex",StringProperty,
         _INTL("Description of the Pokémon as displayed in the Pokédex.")],
     ["Type1",TypeProperty,
         _INTL("Pokémon's type.  If same as type 2, this Pokémon has a single type.")],
     ["Type2",TypeProperty,
         _INTL("Pokémon's type.  If same as type 1, this Pokémon has a single type.")],
     ...
     ["Rareness",LimitProperty.new(255),
         _INTL("Catch rate of this species.")],
     ["Happiness",LimitProperty.new(255),
         _INTL("Base happiness of this species.")],
     ["GenderRate",EnumProperty.new([_INTL("AlwaysMale"),_INTL("FemaleOneEighth"),
     ...
     ["GrowthRate",EnumProperty.new([_INTL("Medium"),_INTL("Erratic"),
...
def pbTrainerTypeEditor
  ...
  trainerTypes=[
    ...
    [_INTL("Money per level"),LimitProperty.new(255),
        _INTL("Player earns this amount times the highest level among the trainer type's Pokémon.")],
     ...
     if trtype
       if button==Input::A
         ...
         if selection<0
           ...
           data=[getConstantName(PBTrainers,trtype[0])]
           for i in 2..trtype.length
             data.push(trtype[i])
           end
