...
#    $fps=FPS_Display.new
    $tempary=[commands,showContinue,trainer,framecount,mapid]
    @commands=commands
    @sprites={}
    @viewport=Viewport.new(0,0,Graphics.width,Graphics.height)
    @viewport.z=99998
    i=Kernel.getPlaceInGame[0]
    addBackgroundOrColoredPlane(@sprites,"background","changed_bg/loadbg_"+i.to_s,
  ...
  def pbSetParty(trainer)
    return if !trainer || !trainer.party #boobay
 #     $game_player.character_name=meta[1]
 # $game_player.character_hue=0
 # $PokemonGlobal.playerID=id

    meta=pbGetMetadata(0,MetadataPlayerA) if trainer.trainertype==0
    meta=pbGetMetadata(0,MetadataPlayerB) if trainer.trainertype==1
    meta=pbGetMetadata(0,MetadataPlayerC) if trainer.trainertype==2
    meta=pbGetMetadata(0,MetadataPlayerD) if trainer.trainertype==3
    meta=pbGetMetadata(0,MetadataPlayerE) if trainer.trainertype==178
    meta=pbGetMetadata(0,MetadataPlayerF) if trainer.trainertype==179
   # meta=pbGetMetadata(0,MetadataPlayerA+trainer.metaID)
    if meta
      ...
      filename=meta[1]
      #filename=pbGetPlayerCharset(meta,2,trainer)
      @sprites["player"]=IconSprite.new(64,64,@viewport)
      @sprites["player"].setBitmap("Graphics/Characters/"+filename) #clothcloth
      if trainer && trainer.clothes && trainer.trainertype < 6 || trainer.trainertype==178 || trainer.trainertype==179
          bittrip=BitmapCache.load_bitmap("Graphics/Characters/Clothes/backpack#{trainer.clothes[3]}")
          @sprites["player"].bitmap.blt(0,0,bittrip,Rect.new(0,0,bittrip.width,bittrip.height))
          #        bittrip=BitmapCache.load_bitmap("Graphics/Characters/Clothes/shoes#{trainer.clothes[5]}")
          #    @sprites["player"].bitmap.blt(0,0,bittrip,Rect.new(0,0,bittrip.width,bittrip.height))    

          bittrip=BitmapCache.load_bitmap("Graphics/Characters/Clothes/pants#{trainer.clothes[4]}")
          @sprites["player"].bitmap.blt(0,0,bittrip,Rect.new(0,0,bittrip.width,bittrip.height))    
          
          bittrip=BitmapCache.load_bitmap("Graphics/Characters/Clothes/shirt#{trainer.clothes[2]}")
          @sprites["player"].bitmap.blt(0,0,bittrip,Rect.new(0,0,bittrip.width,bittrip.height))

    
          bittrip=BitmapCache.load_bitmap("Graphics/Characters/Clothes/coat#{trainer.clothes[1]}")
          @sprites["player"].bitmap.blt(0,0,bittrip,Rect.new(0,0,bittrip.width,bittrip.height))
          
          if trainer.trainertype > -1
            bittrip=BitmapCache.load_bitmap("Graphics/Characters/Clothes/hair#{trainer.clothes[5]}"+appendEnd)
            @sprites["player"].bitmap.blt(0,0,bittrip,Rect.new(0,0,bittrip.width,bittrip.height))
          end
          append=""
...
class PokemonLoad
  ...
  def pbStartLoadScreen(savenumber=0,damian=[true])
    #File.new "Graphics/Icons/icon10b.txt","w"
    ...
    savefile=RTP.getSaveFileName("Game.rxdata") 
       ...
       #  Kernel.pbMessage("Game_"+savenumber.to_s+".rxdata")

    savefile=RTP.getSaveFileName("Game_"+savenumber.to_s+".rxdata")
    end
    ...
    if safeExists?(savefile)
      ...
      commands[cmdOtherSaves=commands.length]=_INTL("Save Files") #pbStartLoadScreen(savenumber=0)
      commands[cmdMysteryGift=commands.length]=_INTL("Mystery Gift") if (trainer.mysterygiftaccess rescue false)
#      commands[cmdDirectGift=commands.length]=_INTL("Direct Gift") if (trainer.mysterygiftaccess rescue false)
       commands[cmdControls=commands.length]=_INTL("Controls")
       commands[cmdEgg=commands.length]=_INTL("Generate Egg")
      ...
      commands[cmdOtherSaves=commands.length]=_INTL("Save Files") #pbStartLoadScreen(savenumber=0)
      commands[cmdControls=commands.length]=_INTL("Controls")
     # Kernel.pbMessage(commands.length.to_s)
      commands[cmdOption=commands.length]=_INTL("Options")
    end
    ...
    loop do
      ...
      if cmdOtherSaves>=0 && command==cmdOtherSaves                  
        tempsave=(Kernel.pbMessage(_INTL("Which save file would you like to load?"),[_INTL("Save File 1"),_INTL("Save File 2"),_INTL("Save File 3")],0))
        @scene.pbEndScene

        pbSetUpSystem(tempsave,true)
        scene=PokemonLoadScene.new
        screen=PokemonLoad.new(scene)
        screen.pbStartLoadScreen(tempsave)
        return
        #   sscene=PokemonLoadScene.new
        #     sscreen=PokemonLoad.new(sscene)
        #     sscreen.pbStartLoadScreen(tempsave)
      end
      if cmdControls>=0 && command==cmdControls
      ...
      if cmdContinue>=0 && command==cmdContinue
        ...
        File.open(savefile){|f|
           ...
           $game_variables[96]=savenumber
           #$game_variables[196]=nil
           
           if !pbGet(196).is_a?(Array)
             pbSetRandomMart
           end
           if !pbGet(198).is_a?(Array)
             $game_variables[198]=[]
             10.times do
               $game_variables[198].push(0)
             end
           end
           ...
           #$game_variables[189]=15 if $game_variables[189]!=11
           #$game_variables[189]=0
           $game_variables[80][Kernel.gnfu("EV Trainer")]=false if !$game_switches[698] && $game_variables[80].is_a?(Array)
           $game_switches[698]=true
        ...
        if (cmdContinue>=0 && Kernel.pbConfirmMessageSerious("A save file already exists. Are you sure you wish to start a new game and overwrite it?")) || cmdContinue<0 #nono
        @scene.pbEndScene
        ...
        $game_map.autoplay
        $game_map.update
        $game_switches[322] = true if File.exists?(RTP.getSaveFileName("Game.rxdata")) || File.exists?(RTP.getSaveFileName("Game_1.rxdata")) || File.exists?(RTP.getSaveFileName("Game_2.rxdata")) || safeExists?(RTP.getSaveFileName("Game.rxdata")) || safeExists?(RTP.getSaveFileName("Game_1.rxdata")) || safeExists?(RTP.getSaveFileName("Game_2.rxdata"))
        return
      end
    #elsif cmdDirectGift>=0 && command==cmdDirectGift
        ...
        pbFadeOutIn(99999){
          ...
          @scene.pbEndScene#;@scene.pbCloseScene
          GTS.open
          damian[0]=false
          return
      #    $scene=pbCallTitle
          ...
          begin
            ...
            File.open(RTP.getSaveFileName("Game.rxdata"),"wb"){|f|
               14.times {|i| Marshal.dump(savedata[i],f) }
                        ...
                        File.open(RTP.getSaveFileName("Game_"+savenumber.to_s+".rxdata"),"wb"){|f|
               14.times {|i| Marshal.dump(savedata[i],f) }
            }
    end
          rescue; end
