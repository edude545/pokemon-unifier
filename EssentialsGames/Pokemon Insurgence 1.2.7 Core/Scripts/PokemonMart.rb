...
class PokemonMartAdapter
  ...
  def canSell?(item)
    return false if item==PBItems::CRYSTALPIECE
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if pbIsImportantItem?(item)
        if !pbConfirm(_INTL("Certainly. You want the {1}.\r\nThat will be ${2}. OK?",itemname,price))
          next
        end
        quantity=1
      else
...
def pbRandomPokemonMart(stocknum,speech=nil,cantsell=false)
  stock=$game_variables[196][stocknum]
  for i in 0...stock.length
