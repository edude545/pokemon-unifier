...
# To be used in a script event command.
  def pbSetSelfSwitch(event,swtch,value,mapforce=0)
    #  Kernel.pbMessage(event.to_s)
   #   Kernel.pbMessage(swtch)
  #    Kernel.pbMessage(value.to_s)
    mapvalue=@map_id
    mapvalue=mapforce if mapforce != 0
    $game_self_switches[[mapvalue,event,swtch]]=value
...
#raise message
  #  raise "dick"  #if message.include?("Can't use that here")
  ret=0
...
def Kernel.pbCreateMessageWindow(viewport=nil,skin=nil)
  if $Bubble==2 # Message window set to floating bubble.
    if $game_player.direction==8 && !$game_switches[63]# Player facing up, message window top. 
    ...
    case $game_system.message_position
      ...
      when 2
       ...
       if $Bubble==1
         ...
         if $game_player.direction==8 && !$game_switches[63]
           @Restriction = Viewport.new(0, 0, Graphics.width, 280)
           msgwindow.y = 6
         else
...
def Kernel.pbMessageDisplayr(msgwindow,message,letterbyletter=true,commandProc=nil)
    begin 
      Kernel.pbMessageRedirect(msgwindow,message,letterbyletter=true,commandProc=nil)
    rescue Exception
...
def Kernel.pbMessageDisplay(msgwindow,message,letterbyletter=true,commandProc=nil)
  ...
  if !$PokemonSystem || ($PokemonSystem && (!$PokemonSystem.hiContrast || $PokemonSystem.hiContrast==0))
