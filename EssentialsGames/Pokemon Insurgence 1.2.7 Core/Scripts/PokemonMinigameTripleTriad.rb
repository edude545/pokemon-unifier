  ...
  #            Kernel.pbMessage("dana1")
    ...
    if oppdeck && oppdeck.is_a?(Array) && oppdeck.length==self.maxCards # Preset
      ...
      while candidates.length<900
        ...
        if candidates.length<900 && $Trainer && $Trainer.owned[card]
          # Add again if Trainer owns the species
          candidates.push([card,total])
        end
      end
  #        Kernel.pbMessage("dana")
...
=begin
      if minIndex==10
        candidates.reverse!
        for i in 21..candidates.length-1
            candidates[i]=nil
        end
        candidates.compact!
       #   for i in 0..20   
...
=begin
  ...
  #    end
    ...
    Kernel.pbMessage(opponentCards.length.to_s)
