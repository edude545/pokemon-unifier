...
################################################################################
MYSTERYGIFTURL = "http://p-insurgence.com/gift_read5.txt"
...
# item: The thing being turned into a Mystery Gift.
def pbEditMysteryGift(type,item,val=0,name="",plural="")
  if type==0   # Pokémon
    ...
    if command>=0 && command<commands.length-1
      ...
      loop do
        name=Kernel.pbMessageFreeText(_INTL("Enter a phrase."),"",false,32)
        break if name!=""
        Kernel.pbMessage(_INTL("The gift must have a phrase for where it was obtained from."))
      end
    end
  elsif type>0   # Item
    ...
    type=Kernel.pbMessageChooseNumber(_INTL("Choose a quantity."),params)
    if type>1 && Kernel.pbConfirmMessage(_INTL("Does this gift need a plural form of its name?"))
      plural=Kernel.pbMessageFreeText(_INTL("Enter a plural form of the gift's name."),name,false,32)
    end
  end
  if val==0
    master=[]; idlist=[]
    ...
    loop do
      ...
      params.setDefaultValue(val)
      params.setCancelValue(0)
      val=Kernel.pbMessageChooseNumber(_INTL("Choose a unique ID for this gift."),params)
      if val==0
        Kernel.pbMessage(_INTL("You must choose a number geater than 0."))
      else
        if idlist.include?(val)
  ...
  loop do
    name=Kernel.pbMessageFreeText(_INTL("Enter a name for the gift."),name,false,32)
    break if name!=""
    Kernel.pbMessage(_INTL("The gift must have a name."))
  end
  return [val,type,item,name,plural]
end
...
def pbCreateMysteryGift(type,item)
  ...
  File.open(_INTL("MysteryGiftMaster.txt"),"wb"){|f|
     f.write(string)
  }
  ...
  loop do
    ...
    if command==-1 || command==commands.length-1
      ...
      begin
        ...
        File.open(_INTL("MysteryGift.txt"),"wb"){|f|
           f.write(string)
        }
      ...
      loop do
        ...
        if cmd==-1 || cmd==cmds.length-1
          ...
          master[command]=pbEditMysteryGift(gift[1],gift[2],gift[0],gift[3],gift[4])
        elsif cmd==2   # Receive
          replaced=false
      ...
      loop do
        ...
        if command==-1 || command==commands.length-1
          ...
          if gift[1]==0
            ...
            sprite.setBitmap(sprintf("Graphics/Icons/item%03d",gift[2]))
          end
...
def pbDownloadDirectGift(trainer)
  sprites={}
  viewport=Viewport.new(0,0,Graphics.width,Graphics.height)
  viewport.z=99999
  addBackgroundPlane(sprites,"background","mysteryGiftbg",viewport)
  ...
  sprites["msgwindow"]=Kernel.pbCreateMessageWindow
  Kernel.pbMessageDisplay(sprites["msgwindow"],_INTL("Searching for a gift.\nPlease wait...\\wtnp[0]"))
  string=$game_variables[166]  
  if string==""
    Kernel.pbMessageDisplay(sprites["msgwindow"],_INTL("No new gifts are available."))
  else
          sprites["msgwindow"].visible=false
            sprite=PokemonSprite.new(viewport)
            sprite.setPokemonBitmap(pkmn.species)
  ...
  if gift[1]==0
    ...
    if $PokemonBag.pbCanStore?(gift[2],gift[1])
      ...
      if $ItemData[item][ITEMUSE]==3 || $ItemData[item][ITEMUSE]==4
        ...
        Kernel.pbMessage(_INTL("\\se[itemlevel]{1} received \\c[2]{2}\\c[0]!\\nIt contained \\c[2]{3}\\c[0].\\wtnp[30]",
           $Trainer.name,itemname,machine))
      elsif isConst?(item,PBItems,:LEFTOVERS)
        Kernel.pbMessage(_INTL("\\se[itemlevel]{1} received some \\c[2]{2}\\c[0]!\\wtnp[30]",$Trainer.name,itemname))
      elsif qty>1
        if gift[4] && gift[4]!=""
          Kernel.pbMessage(_INTL("\\se[itemlevel]{1} received {2} \\c[2]{3}\\c[0]!\\wtnp[30]",$Trainer.name,qty,gift[4]))
        else
          Kernel.pbMessage(_INTL("\\se[itemlevel]{1} received {2} \\c[2]{3}s\\c[0]!\\wtnp[30]",$Trainer.name,qty,itemname))
        end
      else
        if "AEIOUaeiou".include?(itemname[0])
          Kernel.pbMessage(_INTL("\\se[itemlevel]{1} received an \\c[2]{2}\\c[0]!\\wtnp[30]",$Trainer.name,itemname))
        else
          Kernel.pbMessage(_INTL("\\se[itemlevel]{1} received a \\c[2]{2}\\c[0]!\\wtnp[30]",$Trainer.name,itemname))
        end
      end
      $Trainer.mysterygift[index]=[id]
      return true
