    ...
    for hidden in REGIONMAPEXTRAS
      if hidden[0]==region && hidden[1]>0 && $game_switches[hidden[1]]
        ...
        pbDrawImagePositions(@sprites["map2"].bitmap,[
           ["Graphics/Pictures/#{hidden[4]}",hidden[2]*16,hidden[3]*16,0,0,-1,-1]
        ])
      end
    end
    @point=BitmapWrapper.new(20,20)
    @point.fill_rect(0,0,20,20,Color.new(255,0,0))
    @point2=BitmapWrapper.new(20,20)
    @point2.fill_rect(4,0,16,20,Color.new(255,0,0))
    @point3=BitmapWrapper.new(20,20)
    @point3.fill_rect(0,4,20,16,Color.new(255,0,0))
    @point4=BitmapWrapper.new(20,20)
    @point4.fill_rect(4,4,16,16,Color.new(255,0,0))
...
class PokedexFormScene
  def pbStartScene(species)
    ...
    if species <= 925
      ...
      $temp_show_back = false 
    end
    @sprites["back"]=PokemonSprite.new(@viewport) if $temp_show_back
    @sprites["icon"]=PokemonSpeciesIconSprite.new(@species,@viewport) if $temp_show_back
    @sprites["icon"].gender=@gender if $temp_show_back
    @sprites["icon"].form=@form if $temp_show_back
    @sprites["icon"].x=52 if $temp_show_back
    @sprites["icon"].y=290 if $temp_show_back
    pbUpdate
    return true
  end
  ...
  def pbUpdate
    ...
    backBitmap=pbCheckPokemonBitmapFiles([@species,true,(@gender==1),false,@form,false]) if $temp_show_back
    if  $temp_show_back && backBitmap
    ...
    pbPositionPokemonSprite(@sprites["back"],310,98+16+backMetric*2) if $temp_show_back
    @sprites["icon"].update if $temp_show_back
  end
  ...
  def pbChooseForm
    ...
    using(cmdwindow=Window_CommandPokemon.new(commands)) {
       ...
       loop do
         ...
         Input.update
         cmdwindow.update
         @gender=@sprites["icon"].gender=@available[cmdwindow.index][1] if $temp_show_back
         @form=@sprites["icon"].form=@available[cmdwindow.index][2] if $temp_show_back
         pbUpdate
         if Input.trigger?(Input::B)
           pbPlayCancelSE()
           @gender=@sprites["icon"].gender=oldgender if $temp_show_back
           @form=@sprites["icon"].form=oldform if $temp_show_back
           break
         end
