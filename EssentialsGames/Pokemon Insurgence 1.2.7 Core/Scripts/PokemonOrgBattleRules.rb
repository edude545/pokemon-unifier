...
class StandardRestriction
  def isValid?(pokemon)
    ...
    #  return true
    #end
    # Certain named species are banned #blaziken, deoxys, shaymin
    blacklist=[:MEWTWO,:MEW,:LUGIA,:HOOH,:CELEBI,:KYOGRE,:GROUDON,:RAYQUAZA,
    :JIRACHI,:DEOXYS,:DIALGA,:PALKIA,:GIRATINA,:PHIONE,:MANAPHY,:DARKRAI,:SHAYMIN,
    :ARCEUS,:VICTINI,:RESHIRAM,:ZEKROM,:KYUREM,:KELDEO,:MELOETTA,:GENESECT,
    :XERNEAS,:YVELTAL,:ZYGARDE,:DIANCIE,:HOOPA,:VOLCANION,:MISSINGNO,:DELTAMELOETTA,
    :DELTAHOOPA,:UFI]
    for i in blacklist
      return false if isConst?(pokemon.species,PBSpecies,i)
    end
    #return false if isConst?(pokemon.species,PBSpecies,:MELOETTA) && pbHasMove?(pokemon,PBMoves::DARKSONATA)

    #return false if isConst?(pokemon.species,PBSpecies,:SCEPTILE) && isConst?(pokemon.item,PBItems,:SCEPTITE)
    #return false if isConst?(pokemon.species,PBSpecies,:BLAZIKEN) && isConst?(pokemon.item,PBItems,:BLAZIKENITE)
    return false if isConst?(pokemon.species,PBSpecies,:LATIOS) && isConst?(pokemon.item,PBItems,:SOULDEW)
    return false if isConst?(pokemon.species,PBSpecies,:LATIAS) && isConst?(pokemon.item,PBItems,:SOULDEW)
    return false if isConst?(pokemon.species,PBSpecies,:REGIGIGAS) && isConst?(pokemon.item,PBItems,:CRYSTALPIECE)
    #return false if isConst?(pokemon.species,PBSpecies,:BLAZIKEN) && pokemon.ability==2
    #return false if isConst?(pokemon.species,PBSpecies,:DEOXYS) && (pokemon.form==0 || pokemon.form==1)
    #return false if isConst?(pokemon.species,PBSpecies,:SHAYMIN) && (pokemon.form==1)
...
class TypeClause
  def isValid?(pokemon)
   # Kernel.pbMessage("hay")
   # return !pokemon.type == $game_variables[76]
  end
end
...
class LittleCupRestriction
  def isValid?(pokemon)
    ...
    return false if pbHasMove?(pokemon,:SONICBOOM)
    return false if pbHasMove?(pokemon,:DRAGONRAGE)
...
class NoItemClause
  def isValid?(team)
    Kernel.pbMessage("1.16")
    for i in 0...team.length-1
      next if team[i].item==0
      for j in i+1...team.length
        return false if team[i].item!=0
      end
    end
    Kernel.pbMessage("1.161")
    return true
  end

  def errorMessage
    return _INTL("No hold items.")
  end
end
=end
...
class BattleType
  def pbCreateBattle(scene,trainer1,trainer2)
    partyC1=trainer1.party
    trainerC2=trainer2
    if pbGet(1)==7
      partyC2=pbLoadTrainerScaled(ChallengeChampionship.new.getTrainerClass,ChallengeChampionship.new.getTrainerName,5)[2]
      trainerC2=pbLoadTrainerScaled(ChallengeChampionship.new.getTrainerClass,ChallengeChampionship.new.getTrainerName,5)[0]
      else
    partyC2=trainer2.party
      end
    
    return PokeBattle_RecordedBattle.new(scene,
       partyC1,partyC2,trainer1,trainerC2)
  end
end



class BattleTowerClass < BattleType
  def pbCreateBattle(scene,trainer1,trainer2)
    partyC1=trainer1.party
    trainerC2=trainer2
    if pbGet(1)==7
...
class BattlePalaceClass < BattleType
  def pbCreateBattle(scene,trainer1,trainer2)
    partyC1=trainer1.party
    trainerC2=trainer2
    if pbGet(1)==7
...
class BattleArenaClass < BattleType
  def pbCreateBattle(scene,trainer1,trainer2)
    partyC1=trainer1.party
    trainerC2=trainer2
    if pbGet(1)==7
...
class BattlePalace < BattleType
  def pbCreateBattle(scene,trainer1,trainer2) #purrloin1
  #  Kernel.pbMessage("yes") if trainer1
  #  KerneCl.pbMessage("no") if trainer2
    partyC1=trainer1.party
    trainerC2=trainer2
    if pbGet(1)==7
      partyC2=pbLoadTrainerScaled(ChallengeChampionship.new.getTrainerClass,ChallengeChampionship.new.getTrainerName,5)[2]
...
class TypeBattleClause < BattleRule
  #def setRule(battle); battle.rules["typeclause"]=true; end
    def isValid?(pokemon)
  # Kernel.pbMessage(""+pokemon.type1.to_s)
   #Kernel.pbMessage(""+$game_variables[72])
   return pokemon.type1==$game_variables[76] || pokemon.type2==$game_variables[76] # == $game_variables[76]
  end
  end
...
def pbChallengeChampionshipRules(double,openlevel)
  ret=PokemonChallengeRules.new
  if openlevel
    ret.setLevelAdjustment(OpenLevelAdjustment.new(60))
  else
...
def pbPyramidRules(double,openlevel)
  ret=PokemonChallengeRules.new
  if openlevel
    ret.setLevelAdjustment(OpenLevelAdjustment.new(60))
  else
...
def pbBattleFactoryRules(double,openlevel)
  ret=PokemonChallengeRules.new
  if openlevel
    ret.setLevelAdjustment(FixedLevelAdjustment.new(120))
    ret.addPokemonRule(MaximumLevelRestriction.new(120))
  else
