def pbRandomPhoneTrainer
  ...
  for num in $PokemonGlobal.phoneNumbers
    if num[0] && num.length==8 # if visible and a trainer
      ...
      if callerregion[0]!=currentregion[0]   # Can't call if in different region
        next
      end
      temparray.push(num)
    end
...
def pbPhoneRegisterNPC(ident,name,mapid)
  if !$PokemonGlobal.phoneNumbers
    $PokemonGlobal.phoneNumbers=[]
  end
  ...
  Kernel.pbMessage(_INTL("Registered {1} in the Pokégear.",name))
end
...
def pbPhoneRegister(event,trainertype,trainername)
  ...
  trainer=pbFindPhoneTrainer(trainertype,trainername)
  if trainer
    return
  end
  phonenum=[]
...
class PokemonPhoneScene
  def start
    ...
    @sprites["bottom"].text=_INTL("<ac>{1}",mapname)
    ...
    filename=sprintf("Graphics/Characters/trchar%03d",@trainers[0][0])
    ...
    pbActivateWindow(@sprites,"list"){
       ...
       loop do
         ...
         if @sprites["list"].index!=oldindex
           ...
           if trainer.length==4
             filename=sprintf("Graphics/Characters/trchar%03d",trainer[0])
           else
