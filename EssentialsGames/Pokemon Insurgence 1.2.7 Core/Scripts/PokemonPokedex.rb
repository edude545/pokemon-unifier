...
#    for i in 0...$PokemonGlobal.pokedexViable.length
      index=$PokemonGlobal.pokedexViable[0]
      ...
      seen.push($Trainer.pokedexSeen(-1).to_s)
      owned.push($Trainer.pokedexOwned(-1).to_s)
#    end
    ...
    if Input.trigger?(Input::C)
      case @sprites["commands"].index
        when @sprites["commands"].itemCount-1
          ...
          pbPlayDecisionSE()
          $PokemonGlobal.pokedexDex=$PokemonGlobal.pokedexViable[1]
...
=end
  ...
  def pbSearchDexList(params)
    ...
    if params[2]!=0 || params[3]!=0 # Filter by type
      ...
      dexlist=dexlist.find_all {|item|
         ...
         type1=dexdata.fgetb
         #Kernel.pbMessage(_INTL("type1: {1}",type1.to_s))
         type2=dexdata.fgetb
  ...
  def pbChangeToDexEntry(species,shouldDrawSecond=false)
    ...
    textpos=[
       [_ISPRINTF("{1:03d}{2:s} {3:s}",indexNumber," ",PBSpecies.getName(species)),
          244,40,0,Color.new(248,248,248),Color.new(0,0,0)], #244,40,0
    ...
    if $Trainer.owned[species]
      ...
      if shouldDrawSecond
      drawTextEx(@sprites["overlay"].bitmap, #42,240
         4,240,Graphics.width-(8),4,dexentry,basecolor,shadowcolor,true) #42*2
       else
               drawTextEx(@sprites["overlay"].bitmap, #42,240
         4,240,Graphics.width-(8),4,dexentry,basecolor,shadowcolor)
       end
       
      if pbResolveBitmap(sprintf("Graphics/Icons/Footprints/footprint%03d",species))
        footprint=BitmapCache.load_bitmap(sprintf("Graphics/Icons/Footprints/footprint%03d",species))
  ...
  def pbStartDexEntryScene(species,shouldDrawSecond=false)     # Used only when capturing a new species
    @sprites={}
  #  $game_variables[131]=species
         ...
         if Input.trigger?(Input::C)
           ...
           if command==[0,0]
             ...
             if dexlist.length==0
               ...
               # Update the slider
               ycoord=62
               if @sprites["pokedex"].itemCount>1 #188.0
                 ycoord+=188.0 * @sprites["pokedex"].index/(@sprites["pokedex"].itemCount-1)
               end
               @sprites["slider"].y=ycoord
               @searchResults=true
...
class PokemonPokedex
  ...
  def pbDexEntry(species)
    @scene.pbStartDexEntryScene(species,false)
    @scene.pbMiddleDexEntryScene
    @scene.pbEndScene
   # @scene.pbStartDexEntryScene(species,true)
   # @scene.pbMiddleDexEntryScene
   # @scene.pbEndScene
  end
