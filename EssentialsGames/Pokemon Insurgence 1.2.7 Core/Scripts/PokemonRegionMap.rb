...
class PokemonRegionMapScene
  ...
  def pbStartScene(aseditor=false,mode=0)
   ...
   #" @map[1] = 1 if $game_switches[227]
    ...
    if playerpos && mapindex==playerpos[0]
      ...
      @sprites["player"].setBitmap(sprintf("Graphics/Pictures/mapPlayer%03d",$Trainer.trainertype))
      @sprites["player"].x=-8+(@mapX*16)+(Graphics.width-@sprites["map"].bitmap.width)/2
      @sprites["player"].y=-8+(@mapY*16)+(Graphics.height-@sprites["map"].bitmap.height)/2
    end
    if mode>0
      k=0
      for i in LEFT..RIGHT
        for j in TOP..BOTTOM
          ...
          if healspot && $PokemonGlobal.visitedMaps[healspot[0]]
            ...
            @sprites["point#{k}"].x=-8+(i*16)+(Graphics.width-@sprites["map"].bitmap.width)/2
            @sprites["point#{k}"].y=-8+(j*16)+(Graphics.height-@sprites["map"].bitmap.height)/2
    ...
    @sprites["cursor"].play
    @sprites["cursor"].x=-8+(@mapX*16)+(Graphics.width-@sprites["map"].bitmap.width)/2
    @sprites["cursor"].y=-8+(@mapY*16)+(Graphics.height-@sprites["map"].bitmap.height)/2
    @changed=false
  ...
  def pbMapScene(mode=0)
    ...
    #    Kernel.pbMessage("1") if healspot
     #   Kernel.pbMessage("2") if $PokemonGlobal.visitedMaps[healspot[0]]
        if healspot && $PokemonGlobal.visitedMaps[healspot[0]]
          return healspot
        end
