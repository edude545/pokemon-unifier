...
Events.onWildBattleOverride+= proc { |sender,e|
   ...
   next if !$PokemonTemp.nowRoaming || $PokemonTemp.roamerIndex==nil ||
           !$PokemonGlobal.roamEncounter
  ...
  for i in 0...RoamingSpecies.length
    ...
    if $game_switches[poke[2]] && $PokemonGlobal.roamPokemon[i]!=true
      ...
      if possiblemaps.include?(currentArea) && pbRoamingMethodAllowed(poke[3])
        ...
        roam.push([species,poke[1],poke[4]])
      end
    end
  end
  if roam.length>0
    ...
    $PokemonTemp.roamerIndex=rnd
    if roamEncounter[2] && roamEncounter[2]!=""
      $PokemonGlobal.nextBattleBGM=roamEncounter[2]
    end
    return [roamEncounter[0],roamEncounter[1]]
  end
  return encounter
})
