...
module PokemonSelection
  def self.rules(min=1, max=6, canCancel=false, acceptFainted=false)
    ...
    ret.addPokemonRule(TypeBattleClause.new)
   # ret.addPokemonRule(MaximumLevelRestriction.new(pbGet(71)[1]))

#    Kernel.pbMessage(""+pbGet(71)[1].to_s)
#    ret.addLevelRule(1,pbGet(71)[1],100)
    return ret
  end
...
class PokemonRuleSet # Redefined to fix a bug
  def hasValidTeam?(team)
    ...
    #if @teamRules.length>0
    #Kernel.pbMessage(_INTL("{1}",@teamRules.length))
    #  pbEachCombination(team,teamNumber){|comb|
    #     if isValid?(comb)
    #       return true
    #     end
    #  }
    #Kernel.pbMessage("1.6")
    #  return false
    #end
    return true
  end
