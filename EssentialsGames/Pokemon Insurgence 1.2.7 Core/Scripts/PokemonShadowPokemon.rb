...
def pbPurify(pokemon,scene)
  if pokemon.heartgauge==0 && pokemon.shadow
    ...
    if newlevel==curlevel
      ...
      pbChangeLevel(pokemon,newlevel,scene,false) # for convenience
      pokemon.exp=newexp
    end
...
def pbRaiseHappinessAndReduceHeart(pokemon,scene,amount)
  if !pokemon.shadow
    ...
    return false
  elsif pokemon.happiness==255 && pokemon.heartgauge==0
    scene.pbDisplay(_INTL("It won't have any effect."))
    return false
...
class PokeBattle_Pokemon
  ...
  def hypermode
    (self.heartgauge==0 || self.hp==0) ? false : @hypermode
  end
  ...
  def isShadow?
    #Kernel.pbMessage("1")
    return true if(@heartgauge && @heartgauge>=0 && @shadow)
    return false
  end
  def isShadowForMove?
      return @shadow
...
class PokeBattle_Battler
  ...
  def pbInitPokemon(*arg)
    if self.pokemonIndex>0 && self.inHyperMode? && self.hp>0
      ...
      self.pokemon.heartgauge-=50
      self.pokemon.heartgauge=0 if self.pokemon.heartgauge<0
    end
...
#      end
      self.pokemon.heartgauge-=30 if @battle.pbOwnedByPlayer?(@index)
      self.pokemon.heartgauge=0 if self.pokemon.heartgauge<0
    end
  end
...
class PokeBattle_Move_12E < PokeBattle_Move
  def pbEffect(attacker,opponent)
    ...
    for i in 0...4
      ...
      affected.push(i) if @battle != nil && @battle.battlers[i] != nil && @battle.battlers[i].hp != nil && @battle.battlers[i].hp>1
    end
...
class PokeBattle_Move_130 < PokeBattle_Move
  def pbEffect(attacker,opponent)
    ...
    if opponent.damagestate.calcdamage>0
      if !isConst?(attacker.ability,PBAbilities,:ROCKHEAD)
        attacker.pbReduceHP([1,(attacker.hp/2).floor].max)
