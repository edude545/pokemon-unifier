...
class PokemonStorage
  ...
  def pbMoveCaughtToParty(pkmn)
    ...
    self.party[self.party.length]=pkmn if !$game_switches[347]
  end
...
class PokemonStorageScreen
  ...
  def pbStore(selected,heldpoke)
    ...
    if pbAbleCount<=1 && pbAble?(@storage[box,index]) && !heldpoke
      pbDisplay(_INTL("That's your last Pokémon!"))
          elsif @storage[box,index]==nil
      pbDisplay(_INTL("Please deposit it manually."))
  ...
  def pbWithdraw(selected,heldpoke)
    ...
    if heldpoke
      heldpoke.heal if !$game_switches[71]
  ...
  def pbPlace(selected)
    ...
    @heldpkmn.heal if !$game_switches[71]
...
### MOVE #######################################################################
      ...
      loop do
        selected=@scene.pbSelectBox(@storage.party)
        if selected && selected[0]==-3 # Close box
          if $game_switches[356] && pbAllFainted && !pbHeldPokemon
            if $Trainer.party[0].eggsteps<1
             pbDisplay(_INTL("Switch out your Egg Token for an Egg in the box!"))
            next
            end
            end
          if pbHeldPokemon
...
if $Trainer.party[0].eggsteps<1
          ...
          #boolie = pbHeldPokemon && pbHeldPokemon.obtainMode!=1
          if selected[0]<=19 && selected[0]>=0 && $game_switches[356] && !(pbHeldPokemon && pbHeldPokemon.obtainMode!=1 && !pbHeldPokemon.egg? && @storage[selected[0],selected[1]].egg?)
            #!boolie
      #      Kernel.pbMessage(selected[0].to_s)
            Kernel.pbMessage("You cannot interact with this box in an Egglocke.")
            else
          pokemon=@storage[selected[0],selected[1]]
          commands=[
             _INTL("Move"),
          ...
          case command
            when 0 # Move/Shift/Place
              if @heldpkmn && pokemon
                pokemon.eggswitchlvl=@heldpkmn.level if $game_switches[356]
                pbSwap(selected)
              elsif @heldpkmn
                pbPlace(selected)
              else
              ...
              when 0
                ...
                loop do
                  ...
                  if cmd2==-1
                    ...
                    f=Kernel.pbMessageChooseNumber(
                       _INTL("Set the EV for {1} (max. 252).",stats[cmd2]),params)
                    pkmn.ev[cmd2]=f
                    pkmn.calcStats
                    @scene.pbHardRefresh
                  end
...
class PokemonStorageScene
  ...
  def pbBoxName(helptext,minchars,maxchars)
    ...
    ret=pbEnterText(helptext,minchars,maxchars)
    if ret.length>0
      @storage[@storage.currentBox].name=ret
    end
    
    newbox=PokemonBoxSprite.new(@storage,@storage.currentBox,@boxviewport)
    Graphics.frame_reset
    @sprites["box"].dispose
    @sprites["box"]=newbox
  ...
  def pbUpdateOverlay(selection,party=nil)
    ...
    pbDrawTextPositions(overlay,textstrings)
    if pokemon.isShiny? && !pokemon.egg?
...
def Kernel.pbGetStorageCreator
  creator=pbGetMessage(MessageTypes::StorageCreator,0)
  if !creator || creator==""
    creator=_INTL("Bill")
  end
  return creator
  ...
  def access
    ...
    loop do
      command=Kernel.pbShowCommandsWithHelp(nil,
         ...
         _INTL("Deposit Pokémon"),
         _INTL("Withdraw Pokémon"),
         _INTL("Sort Boxed Pokémon"),
         _INTL("See ya!")],
         [_INTL("Move Pokémon stored in Boxes to your party."),
         _INTL("Store Pokémon in your party in Boxes."),
         _INTL("Organize the Pokémon in Boxes and in your party."),
         _INTL("Sort the Pokémon in Boxes and in your party."),
      ...
      if command>=0 && command<3
        ...
        for i in 0...$PokemonStorage.maxBoxes
          ...
          for j in 0...$PokemonStorage.maxPokemon(i)
            counter += 1 if $PokemonStorage[i][j] != nil
          end
          if counter>1
            boxOpt.push(i)
          end
        ...
        if boxOpt.length<=0
          Kernel.pbMessage("There are no Pokemon to sort!")
          break
        end
        v1=Kernel.pbMessage("Would you like to sort the entire PC or just one box?",
        ["PC","Box"])
        boxChoice=-1
          boxOpt=[]
          boxOptTxt=[]
        ...
        if v1==1
          ...
          for k in 0...boxOpt.length
            boxOptTxt.push($PokemonStorage[boxOpt[k]].name)
          end
          boxChoice=Kernel.pbMessage("Which box?",boxOptTxt)

        end
        val= v1==1 ? boxOpt[boxChoice] : -1
        
        v=Kernel.pbMessage(_INTL("How should they be sorted?"),
        [_INTL("(Cancel)"),_INTL("Alphabetical (Species)"),
        _INTL("Alphabetical (Nickname)"),_INTL("Dex Number"),_INTL("Type")])
        if v==0
          next
        else
          Kernel.sortBoxes(v-1,val)
          Kernel.pbMessage("...")
          Kernel.pbMessage("...the PC was sorted alphabetically by species name!") if v==1
          Kernel.pbMessage("...the PC was sorted alphabetically by nickname") if v==2
          Kernel.pbMessage("...the PC was sorted by National Dex number!") if v==3
          Kernel.pbMessage("...the PC was sorted by type!") if v==4
        end
...
def mergeBoxes
  $PokemonStorageNew = PokemonStorage.new(48)
  for i in 0..23
    for j in 0..29
      $PokemonStorageNew[i][j]=$PokemonStorage[i][j]
    end
  end
  for i in 0..23
    for j in 0..29
      $PokemonStorageNew[i+24][j]=$game_variables[59][i][j]
    end
  end
  $PokemonStorage=$PokemonStorageNew
  $PokemonStorageNew = nil
end

def pbPokeCenterPC
  if $game_switches[346] || $game_switches[347]
    Kernel.pbMessage(_INTL("The PC couldn't be accessed due to the Solorun."))
  else
    Kernel.pbMessage(_INTL("\\se[computeropen]{1} booted up the PC.",$Trainer.name))
    loop do
