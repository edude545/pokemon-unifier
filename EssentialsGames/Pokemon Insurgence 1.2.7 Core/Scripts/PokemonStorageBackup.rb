...
class PokemonStorageScreen
  ...
  def pbStore(selected,heldpoke)
    ...
    if !@storage[box,index] || @storage[box,index]=nil || @storage[box,index].mail == nil
        @storage[box,index] = 0
    end
    
    if !@storage[box,index] || @storage[box,index]=nil
        pbDisplay(_INTL("Pokemon could not be stored. Try moving it manually."))
        else
    if pbAbleCount<=1 && pbAble?(@storage[box,index]) && !heldpoke
     pbDisplay(_INTL("That's your last Pokémon!")) #els
    else
      loop do
        ...
        when 12
          ...
          loop do
            ...
            # Rename
              ...
              newname=pbEnterText(_INTL("{1}'s nickname?",speciesname),0,10)
