...
def pbTilesetWrapper
  return PokemonDataWrapper.new(
     ...
     Proc.new{
        Kernel.pbMessage(
           _INTL("The editor has detected that the tileset data was recently edited in RPG Maker XP.")
        )
        next !Kernel.pbConfirmMessage(
           _INTL("Do you want to load those recent edits?")
        )
     }
  )
end
...
class PokemonTilesetScene
  def pbUpdateTileset
    ...
    textpos=[]
    @sprites["tileset"].src_rect=Rect.new(0,@y,256,Graphics.height-64)
    tilesize=@tileset.terrain_tags.xsize
    for yy in 0...(Graphics.height-64)/32
      ypos=(yy+(@y/32))*8+384
      next if ypos>=tilesize
      for xx in 0...8
        terr=ypos<384 ? @tileset.terrain_tags[xx*48] :
           @tileset.terrain_tags[ypos+xx]
        if ypos<384
    ...
    @sprites["overlay"].bitmap.fill_rect(@x,0,32,2,Color.new(255,0,0))
    @sprites["overlay"].bitmap.fill_rect(@x,0,2,32,Color.new(255,0,0))
    @sprites["overlay"].bitmap.fill_rect(@x,30,32,2,Color.new(255,0,0))
    @sprites["overlay"].bitmap.fill_rect(@x+30,0,2,32,Color.new(255,0,0))
  ...
  def pbChooseTileset
    ...
    for i in 1...@tilesetwrapper.data.length
      commands.push(_ISPRINTF("{1:03d} {2:s}",i,@tilesetwrapper.data[i].tileset_name))
    end
  ...
  def pbStartScene
    ...
    @tileset=@tilesetwrapper.data[36]
    @tilehelper=TileDrawingHelper.fromTileset(@tileset)
    @sprites={}
    @sprites["title"]=Window_UnformattedTextPokemon.new(_INTL("Tileset Ed (PgUp/PgDn: SCROLL; Z/F5: MENU)"))
    ...
    loop do
      ...
      if Input.trigger?(Input::F5)
        ...
        if Kernel.pbConfirmMessage(_INTL("Save changes?"))
          ...
          Kernel.pbMessage(_INTL("To ensure that the changes remain, close and reopen RPGXP for best results."))
        end
