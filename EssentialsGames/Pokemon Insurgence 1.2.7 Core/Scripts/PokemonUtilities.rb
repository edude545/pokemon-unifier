...
################################################################################
  def pbChangePlayer(id,specificpoke=0)
    return false if id<0 || id>=19
    meta=pbGetMetadata(0,MetadataPlayerA+id)
    return false if !meta
    $Trainer.trainertype=meta[0] if $Trainer
    $game_player.character_name=meta[1] if specificpoke==0
    $game_player.character_name=specificpoke if specificpoke!=0
    $game_player.character_hue=0
    $PokemonGlobal.playerID=id
  #Kernel.pbMessage("Change: "+$PokemonGlobal.playerID.to_s)
  end

def pbGetPlayerGraphic
  id=$PokemonGlobal.playerID
  return "" if id<0 || id>=15
  meta=pbGetMetadata(0,MetadataPlayerA+id)
  return "" if !meta
  return sprintf("../Characters/trainer%03d",meta[0])
end

def pbGetPlayerWalkingGraphic
  id=$PokemonGlobal.playerID
  return "" if id<0 || id>=15
  meta=pbGetMetadata(0,MetadataPlayerA+id)
  return "" if !meta
  return sprintf("../Characters/trchar%03d",meta[0])
end
def pbGetPlayerTrainerType
  id=$PokemonGlobal.playerID
  return 0 if id<0 || id>=15
  meta=pbGetMetadata(0,MetadataPlayerA+id)
  return 0 if !meta
  return meta[0]
end
...
def pbTrainerName(name=nil)
  ...
  trname=name
  if trname==nil
    trname=pbEnterText("Your name?",0,8)
...
module ItemStorageHelper
  ...
  def self.pbStoreItem(items,maxsize,maxPerSlot,item,qty,sorting=false)
    ...
    for i in 0...maxsize
      ...
      if !itemslot
        ...
        qty-=items[i][1]
        if sorting && pbGetPocket(item)==5
          items.sort! if POCKETAUTOSORT[$ItemData[item][ITEMPOCKET]]                                                                                  # ADDED
        end
        return true if qty==0
      elsif itemslot[0]==item && itemslot[1]<maxPerSlot
        newamt=itemslot[1]
  ...
  def self.pbMassSort(items)
    return true if POCKETAUTOSORT[$ItemData[items[0][0]][ITEMPOCKET]]
    for i in 0...items.length
      #Kernel.pbMessage(_INTL("{1}",items[i][1]))
      if Kernel.pbIsMegaStone?(items[i][0]) || Kernel.pbIsKeyItem?(items[i][0])
        #Kernel.pbMessage("2")
        #$PokemonGlobal.pcItemStorage.pbStoreItem(items[i][0],items[i][1])
        amt=items[i][1]
        id=items[i][0]
        items[i][1]-=amt
        items[i]=nil
        #items[i][1]+=amt
        #$PokemonBag.pbDeleteItem(items[i][0],amt)
        $PokemonBag.pbStoreItem(id,amt)
          #Kernel.pbMessage("3")
      end
    end
    items.compact!
    items.sort!{ |a,b| PBItems.getName(a[0]) <=> PBItems.getName(b[0]) }
    return false
  end
end
...
def pbLoadRpgxpScene(scene,shouldFadeOut=true)
  return if !$scene.is_a?(Scene_Map)
  oldscene=$scene
  $scene=scene
  Graphics.freeze
  oldscene.disposeSpritesets if shouldFadeOut
  visibleObjects=pbHideVisibleObjects if shouldFadeOut
  Graphics.transition(15) if shouldFadeOut
  Graphics.freeze
  ...
  Graphics.transition(15) if shouldFadeOut
  Graphics.freeze
  oldscene.createSpritesets if shouldFadeOut
  pbShowObjects(visibleObjects) if shouldFadeOut
...
def pbExclaim(event,id=EXCLAMATION_ANIMATION_ID)
  if !$game_switches[693]
    sprite=$scene.spriteset.addUserAnimation(id,event.x,event.y)
...
def pbCryFrameLength(pokemon,pitch=nil)
  ...
  if pokemon.is_a?(Numeric)
    ...
    pkmnwav=pbResolveAudioSE(pkmnwav)
    ...
    if pokemon.respond_to?("chatter") && pokemon.chatter
      ...
      pkmnwav=sprintf("%03dCry_%d",pokemon.species,(pokemon.form rescue 0))
      pkmnwav=pbResolveAudioSE(pkmnwav)
      if !pkmnwav
        pkmnwav=sprintf("%03dCry",pokemon.species)
        pkmnwav=pbResolveAudioSE(pkmnwav)
      end
      playtime=getPlayTime(pkmnwav) if pkmnwav
    end 
...
def pbPlayTaxi(volume=90,pitch=nil)
  pkmnwav=sprintf("Pikapad")
  pbSEPlay(RPG::AudioFile.new(pkmnwav,volume,pitch ? pitch : 100)) rescue nil
end
...
def pbGetWildBattleBGM(species)
  ...
  ret=pbStringToAudioFile("legendary_music.ogg") if $game_map.map_id==463 || $game_map.map_id==541
...
#    end
 # end
  randomint = rand(3)
#  Kernel.pbMessage(""+randomint.to_s) if !ret
  ret=pbStringToAudioFile("trainergeneric") if !ret #&& randomint == 0
#    ret=pbStringToAudioFile("005-Boss01_2") if !ret && randomint == 1
 # ret=pbStringToAudioFile("005-Boss01_3") if !ret && randomint == 2
...
def pbGetTrainerVictoryME(trainer) # can be a PokeBattle_Trainer or an array of PokeBattle_Trainer
  ...
  pbRgssOpen("Data/trainertypes.dat","rb"){|f|
     ...
     for i in 0...trainerarray.length
       trainertype=trainerarray[i].trainertype
       trainertype=0 if trainertype.is_a?(String)
       if trainertypes[trainertype]
...
def pbCreatePokemon
  ...
  pbAutoLearnMove($Trainer.party[1],PBMoves::FLY)
  pbAutoLearnMove($Trainer.party[2],PBMoves::FLASH)
  pbAutoLearnMove($Trainer.party[2],PBMoves::TELEPORT)
  pbAutoLearnMove($Trainer.party[3],PBMoves::SURF)
  pbAutoLearnMove($Trainer.party[3],PBMoves::DIVE)
  pbAutoLearnMove($Trainer.party[3],PBMoves::WATERFALL)
  pbAutoLearnMove($Trainer.party[4],PBMoves::DIG)
  pbAutoLearnMove($Trainer.party[4],PBMoves::CUT)
  pbAutoLearnMove($Trainer.party[4],PBMoves::HEADBUTT)
  pbAutoLearnMove($Trainer.party[4],PBMoves::ROCKSMASH)
  pbAutoLearnMove($Trainer.party[5],PBMoves::SOFTBOILED)
  pbAutoLearnMove($Trainer.party[5],PBMoves::STRENGTH)
  pbAutoLearnMove($Trainer.party[5],PBMoves::SWEETSCENT)
end
...
def pbNickname(pokemon)
  ...
  if Kernel.pbConfirmMessage(_INTL("Would you like to give a nickname to {1}?",speciesname))
    if speciesname.split().last=="♂" || speciesname.split().last=="♀"
      speciesname=PBSpecies.getName(pokemon.species)[0..-2]
    end
    genderSymbol=""
    if pokemon.gender==0
      genderSymbol=" "+_INTL("♂")
    ...
    newname=pbEnterText(_INTL("{1}'s{2} nickname?",speciesname,genderSymbol),0,11)
...
def pbStorePokemon(pokemon)
  ...
  if $game_switches[583]
      $game_variables[155]=pokemon
      while 1==1
          Kernel.pbMessage("Preparing to Wonder Trade...")
          $game_switches[585]=true
          if !$game_switches[585]
            break
          end
        end
      pokemon=$game_variables[155]
    

  end
  if $Trainer.party.length<6 && !$game_switches[347]
...
def pbAddPokemon(pokemon,level=nil,seeform=true,heldItem=nil,gender=nil,move1=nil,move2=nil,move3=nil,move4=nil,ability=nil,form=nil)
  ...
  if $game_switches[321] && $game_switches[49]
    species=Kernel.pbGetRandomPoke
    #pokemon.species=species
    #pkmn=PokeBattle_Pokemon.new(species,level,$Trainer)
    #dexdata=pbOpenDexData
    #pbDexDataOffset(dexdata,species,21)
    pokemon=species#pkmn
  end
  
  if pokemon.is_a?(Integer) && level.is_a?(Integer)
    pokemon=PokeBattle_Pokemon.new(pokemon,level,$Trainer)
    if heldItem!=nil
      pokemon.item=Kernel.pbCheckForRandomItem(heldItem)
    end
    if gender!=nil && !$game_switches[321]
      pokemon.setGender(gender)
    end
    if move1!=nil && !$game_switches[321]
      pokemon.moves[0]=PBMove.new(move1)
    end
    if move2!=nil && !$game_switches[321]
      pokemon.moves[1]=PBMove.new(move2)
    end
    if move3!=nil && !$game_switches[321]
      pokemon.moves[2]=PBMove.new(move3)
    end
    if move4!=nil && !$game_switches[321]
      pokemon.moves[3]=PBMove.new(move4)
    end
    if ability!=nil && !$game_switches[321]
      pokemon.setAbility(ability)
    end
    if form!=nil && !$game_switches[321]
      pokemon.form=form
    end
  end
  speciesname=PBSpecies.getName(pokemon.species)
...
def pbAddPokemonSilent(pokemon,level=nil,seeform=true,pokename=nil,obtaintext=nil)
  return false if !pokemon || pbBoxesFull? || !$Trainer
  if $game_switches[321] && $game_switches[49]
    if pokename==nil
      pokename=pokemon.name if pokemon.name!=nil
    end
    deltaAry=[]
    ...
    #pokemon.species=species
    #if pokemon.is_a?(Integer) && level.is_a?(Integer)
    level=pokemon.level if level==nil
    pkmn=PokeBattle_Pokemon.new(species,level,$Trainer)
    #end
    dexdata=pbOpenDexData
    pbDexDataOffset(dexdata,species,21)
    pkmn.eggsteps=dexdata.fgetw if pokename=="Egg"
    pkmn.hatchedMap=0 if pokename=="Egg"
    pkmn.obtainMode=1 if pokename=="Egg"
    pokemon=pkmn
  end
  ...
  if !$game_switches[321]
    if pokename=="Egg"
      ...
      pokemon.eggsteps=dexdata.fgetw
      dexdata.close
      pokemon.hatchedMap=0
      pokemon.obtainMode=1
    end
  end
  pokemon.obtainText=_INTL(obtaintext.to_s) if obtaintext!=nil
  pokemon.name=_INTL(pokename) if pokename!=nil
  $Trainer.seen[pokemon.species]=true
  $Trainer.owned[pokemon.species]=true
  pbSeenForm(pokemon) if seeform
  if $Trainer.party.length<6 && !$game_switches[347]
...
def pbAddToPartySilent(pokemon,level=nil,seeform=true)
  ...
  $Trainer.owned[pokemon.species]=true
  pbSeenForm(pokemon) if seeform
  $Trainer.party[$Trainer.party.length]=pokemon if !$game_switches[347]
  return true
end

def pbAddForeignPokemon(species,level,ownerName,nickname=nil,ownerGender=0,seeform=true)
...
################################################################################
# Load Pokémon sprites and icons
################################################################################
def pbPokemonBitmapFile(species, shiny, back=false)   # Used by the Pokédex
  if shiny
    # Load shiny bitmap
    return sprintf("Graphics/Battlers/%03ds%s",species,back ? "b" : "")
  else
    # Load normal bitmap
    return sprintf("Graphics/Battlers/%03d%s",species,back ? "b" : "")
  end
end

def pbLoadPokemonBitmap(pokemon, back=false,substitute=false)
  return pbLoadPokemonBitmapSpecies(pokemon,pokemon.species,back,substitute)
end

# Note: Returns an AnimatedBitmap, not a Bitmap
def pbLoadPokemonBitmapSpecies(pokemon, species, back=false, substitute=false)
 # Kernel.pbMessage("1")
  ret=nil
  if substitute    
    bitmapFileName=pbResolveBitmap(sprintf("Graphics/Battlers/sub_front")) if !back
    bitmapFileName=pbResolveBitmap(sprintf("Graphics/Battlers/sub_back")) if back
  elsif pokemon.egg?
  ...
  elsif $game_switches && $game_switches[354] && $Trainer != nil && $Trainer.owned != nil && $Trainer.owned[species]==false
#      Kernel.pbMessage("2")

    bitmapFileName=pbResolveBitmap(sprintf("Graphics/Battlers/000"))
  else
 #     Kernel.pbMessage("3")
...
def pbPokemonIconFile(pokemon)
  bitmapFileName=nil
  if pokemon.egg?
    bitmapFileName=pbResolveBitmap(sprintf("Graphics/Icons/icon%03degg",pokemon.species))
...
def pbCheckPokemonIconFiles(params)
  species=params[0]
  for i in 0...2**(params.length-1)
    ...
    bitmapFileName=pbResolveBitmap(sprintf("Graphics/Icons/icon%03d%s%s%s%s",
       species,
       tgender ? "f" : "",
       tshiny ? "s" : "",
       (tform!="" ? "_"+tform : ""),
...
def pbHealAll
  ...
  for i in $Trainer.party
    i.heal if !$game_switches[71] || i.hp>0
  end
end
...
def pbHasEgg?(species)
  ...
  species2=pbGetNonIncenseLowestSpecies(baby)
  return true if species==species2   # Is an egg species without incense
  return false
end
...
def pbEachBoxedPokemon
  for i in 0...$PokemonStorage.maxBoxes
    for j in 0...$PokemonStorage.maxPokemon(i)
      poke=$PokemonStorage[i][j]
     # Kernel.pbMessage(PBSpecies.getName(poke.species) + i.to_s + " " + j.to_s) if poke
      yield(poke,i) if poke
    end
  end
end
...
def pbChoosePokemon(variableNumber,nameVarNumber,ableProc=nil, allowIneligible=false,gtsParty=nil,gtsArr=nil)
  chosen=0
  pbFadeOutIn(99999){
     scene=PokemonScreen_Scene.new
     if gtsParty
       screen=PokemonScreen.new(scene,gtsParty,true,gtsArr)
       if ableProc
         chosen=screen.pbChooseAblePokemon(ableProc,allowIneligible)      
       else
       
         screen.pbStartScene(_INTL("Choose a Pokémon."),nil,false,true)
         chosen=screen.pbChoosePokemon(false,true)
         screen.pbEndScene
       end
     else       
       ...
       if ableProc
         ...
         screen.pbStartScene(_INTL("Choose a Pokémon."),nil,false,false)
         chosen=screen.pbChoosePokemon(false,false)
         screen.pbEndScene
       end
     end
  }
  ...
  if chosen>=0 && chosen < 6
    if gtsParty
      pbSet(nameVarNumber,gtsParty[chosen].name)
    else
...
def pbCheckMove(move)
  move=getID(PBMoves,move)
  if isConst?(move,PBMoves,:WORMHOLE) && $game_switches[237] && $PokemonBag.pbQuantity(PBItems::STARGATE) > 0
      Kernel.pbMessage("There is a Stargate in your bag!")
      return $Trainer.party[0]
    end
  if isConst?(move,PBMoves,:SECRETPOWER) && $PokemonBag.pbQuantity(PBItems::SKELETONKEY) > 0
      Kernel.pbMessage("There is a Skeleton Key in your bag!")
      return $Trainer.party[0]
  end

 # if isConst?(move,PBMoves,:CUT) && !$game_switches[6]
 #     Kernel.pbMessage("This tree looks cuttable.")
 #     return $Trainer.party[0]
 #   end
...
# Other utilities
################################################################################
def pbTextEntry(helptext,minlength,maxlength,variableNumber=1)
  $game_switches[697]=true
  $game_variables[variableNumber]=pbEnterText(helptext,minlength,maxlength)
  $game_switches[697]=false
  $game_map.need_refresh = true if $game_map
  return $game_variables[variableNumber]
end

def pbMoveTutorAnnotations(move,movelist=nil,zygarde=false)
  ret=[]
  for i in 0...6
    ret[i]=nil
    ...
    isCompat=pbSpeciesCompatible?(species,move,form)
    if zygarde && !isConst?($Trainer.party[i].species,PBSpecies,:ZYGARDE)
      # Zygarde should be the only Pokemon learning moves from the Zygarde Cube
      ret[i]=_INTL("NOT ABLE")
    elsif !$Trainer.party[i].egg? && movelist && movelist.any?{|j| j==species }
      # Checked data from movelist
      ret[i]=_INTL("ABLE")
    elsif !$Trainer.party[i].egg? && pbSpeciesCompatible?(species,move,form)
...
def pbMoveTutorChoose(move,movelist=nil,bymachine=false,zygarde=false)
  ret=false
  pbFadeOutIn(99999){
     scene=PokemonScreen_Scene.new
     movename=PBMoves.getName(move)
     screen=PokemonScreen.new(scene,$Trainer.party)
     annot=pbMoveTutorAnnotations(move,movelist,zygarde)
     ...
     loop do
       ...
       if chosen>=0
         ...
         form=pokemon.form
         isCompat=pbSpeciesCompatible?(species,move,form)
         if zygarde && !isConst?($Trainer.party[chosen].species,PBSpecies,:ZYGARDE)
           Kernel.pbMessage(_INTL("Select Zygarde."))
         elsif pokemon.egg?
         ...
         elsif !pbSpeciesCompatible?(pokemon.species,move,form)
           Kernel.pbMessage(_INTL("{1} is not compatible with {2}.",pokemon.name,movename))
           Kernel.pbMessage(_INTL("{1} can't be learned.",movename))
         else
