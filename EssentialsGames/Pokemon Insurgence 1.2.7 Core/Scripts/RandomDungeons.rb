...
Events.onMapCreate+=proc {|sender, e|
  if $game_map.map_id==706
   ...
   if mapID==749
     for event in map.events.values
       next if !event.name.include?("rand") && !event.name.include?("soar")
     #  tile=pbRandomRoomTile(dungeon,roomtiles)
     #  if tile
       if event.name.include?("rand")
         event.x=rand(200)
         event.y=rand(200)
       elsif event.name.include?("soar")
         event.x=$game_temp.player_new_x
         event.y=$game_temp.player_new_y
       end
       
      # end
      
    end
    $game_variables[156]=150
    $game_player.altitude=pbGet(156)
   ...
   if mapID==676
    ...
    if $game_switches[601]
     ...
     for i in 140...168
       ...
       #   nb=TileDrawingHelper.tableNeighbors(tbl,i,j)
        #  tile=TileDrawingHelper::NeighborsToTiles[nb]
          map.data[i,j,0]=rand(400)#tile+48*(tbl[i,j])
          map.data[i,j,1]=rand(400)
          map.data[i,j,2]=rand(400)
        end
    end
    for i in 146...162
        for j in 169...185
