module MessageConfig
  ...
  def self.pbDefaultSystemFrame
   ...
   return pbResolveBitmap("Graphics/Windowskins/skin29.png")||"" if $PokemonSystem && $PokemonSystem.hiContrast && $PokemonSystem.hiContrast != 0
    return pbResolveBitmap("Graphics/Windowskins/"+MessageConfig::ChoiceSkinName+".png")||""
  end

  def self.pbDefaultSpeechFrame
    return "" if !MessageConfig::TextSkinName
    return pbResolveBitmap("Graphics/Windowskins/textbox20.png")||"" if $PokemonSystem && $PokemonSystem.hiContrast && $PokemonSystem.hiContrast != 0
    return pbResolveBitmap("Graphics/Windowskins/"+MessageConfig::TextSkinName+".png")||""
  end
...
module RTP
  ...
  def self.getAudioPath(filename)
    return self.getPath(filename,["",".mp3",".wav",".wma",".mid",".ogg",".midi",".m4a"])
  end
...
module FileTest
  Image_ext = ['.bmp', '.png', '.jpg', '.jpeg', '.gif', '.tiff']
  Audio_ext = ['.mp3', '.mid', '.midi', '.ogg', '.wav', '.wma','.m4a']
...
#    i = [i] if !i.is_a?(Array)
    ...
    textsize.width*=0.85 if i[0]=="D. Etigirafarigite"
    textsize.width*=0.85 if i[0]=="D. Volcarona Armor"
    textsize.width*=0.89 if (i[0]=="D. Metagrossite (S)" || i[0]=="D. Metagrossite (R)")
    textsize.width*=0.91 if i[0]=="Nocturne Incense"
    x=i[1]
    y=i[2]
...
def pbFadeOutInWhite(z)
  col=Color.new(0,0,0,0)
  viewport=Viewport.new(0,0,Graphics.width,Graphics.height)
  viewport.z=z
  for j in 0..17
    col.set(j*15,j*15,j*15,j*15)
    viewport.color=col
    Graphics.update
    Input.update
  end
  ...
  begin
    ...
    pbPopFade
    for j in 0..17
      col.set((17-j)*15,(17-j)*15,(17-j)*15,(17-j)*15)
      viewport.color=col
      Graphics.update
      Input.update
    end
...
def pbFadeOutAndHide(sprites)
  ...
  pbDeactivateWindows(sprites){
     for j in 0..26
       pbSetSpritesToColor(sprites,Color.new(0,0,0,j*10))
...
def pbFadeInAndShow(sprites,visiblesprites=nil)
  ...
  pbDeactivateWindows(sprites){
     for j in 0..27
       pbSetSpritesToColor(sprites,Color.new(0,0,0,((26-j)*10)))
       block_given? ? yield : pbUpdateSpriteHash(sprites)
     end 
  }
end
...
def pbRgssExists?(filename)
  filename=canonicalize(filename) #notice
...
#  raise file if file.include?("indow")
  file=canonicalize(file)
  if !(safeExists?("./Game.rgssad") || safeExists?("./Game.rgss2a"))
  ...
  str=nil
  begin
    if MBLVAR && !file.include?(".png") && !file.include?(".jpg") && !file.include?(".gif") && !file.include?(".jpeg") && file.include?("Graphics")
      file=file+".png"
    end
...
#        if self.top_row + (self.page_row_max - 1) < (self.row_max - 1) # CHANGED
  ...
  def refresh
    ...
    if @__bitmap
      if !@__bitmap.disposed?
        ...
        dwidth=(Graphics.width/@__sprite.zoom_x+2).to_i
        dheight=(Graphics.height/@__sprite.zoom_y+2).to_i
  ...
  def tileBitmap(dstbitmap,srcbitmap,srcrect)
    ...
    y=top;while y<dstrect.height
      x=left;while x<dstrect.width
        dstbitmap.blt(x,y,srcbitmap,srcrect)
        x+=srcrect.width
      end
      y+=srcrect.height
    end
